...
** If the check is ok, _{payment-gateway}_ returns the ``payment-redirect-url`` with a list of available IPP banks, programs and tenures.
...
. _{payment-gateway}_ checks the IPP eligibility of
  - MAID
  - credit card number
  - consumer's country
...
Non-customizable IPPs are predefined. Consumers are prompted to choose from various installment programs. These programs are retrieved with an initial payment request. This request must contain your Merchant Account ID (MAID). With your MAID, _{payment-gateway}_ checks if "onus" and "IPP" are enabled for your merchant account, so make sure that your merchant account has been set up for IPP by <<ContactUs, merchant support>>. 
...
. Clicking "Submit/Pay" calls the <<PPv2_Seamless_Integration_Example_Submit,  ``WPP.seamlessSubmit``>> script:
+
[source,js,subs=attributes+]
----
WPP.seamlessSubmit({
...
+
//NOTE: To retrieve all available IPP options, send a request to the _{payment-gateway}_ <<GeneralPlatformFeatures_IPP_InstallmentCalculator, Installment Calculator>>.
NOTE: To retrieve all available IPP options, send a request to the <<GeneralPlatformFeatures_IPP_InstallmentCalculator, Installment Calculator>>.

[#PPv2_CC_IPP_seamlessCustomizable_Workflow]
[discrete]
===== Workflow

. -> Send an <<GeneralPlatformFeatures_IPP_InstallmentCalculator, Installment Calculator>> request to retrieve the available IPPs.
+
////
+
[#PPv2_CC_IPP_seamlessCustomizable_Samples_IPPSRequest]
.Sample IPPS Request
Endpoint: \http://d-ipps-app01.wirecard.sys:9000/v2/   engine/rest/utils/installmentcalculator +
Authentication:
+
[source,js,subs=attributes+]
----
{
  "gatewayTransactionId": "xxxxxxxxxxxxxxxxxxxxxxx",
  "gatewayMerchantId": xxxxxxxxxxxxxxxxxxxxxxx,
  "pan": xxxxxxxxxxxxxxxxxxxxxxx
}
----
+
[#PPv2_CC_IPP_seamlessCustomizable_IPPSRequestFields]
[%autowidth, cols="m,,,"]
...
|String
|
|Your unique MAID configured for IPP.
|pan
|String
|
|
|===
//// 
. <<GeneralPlatformFeatures_IPP_InstallmentCalculator, Installment Calculator>> returns a list of available IPPs. Depending on your merchant configuration, the IPPs in this response are specified by either a single unique ``installment-plan-id`` or by a set of ``program-id``, ``plan-code`` and ``tenure``.
+
////
[#PPv2_CC_IPP_seamlessCustomizable_Samples_IPPSResponse]
.Sample IPPS Response
[source,js,subs=attributes+]
----
{
  "gatewayTransactionId": "xxxxxxxxxxxxxxxxxxxxxxx",
  ...
  "installmentOptions": [
    {
      "programId": "12345678A",
      "programName": "Installment Payment Plan Example",
      "tenors": [
        {
          "tenor": 3,
          "providerId": "string",
          "processingMerchantId": xxxxxxxxxxxxxxxxxxxxxxx
        }
      ]
    }
  ]
...
----
+
[#PPv2_CC_IPP_seamlessCustomizable_IPPSResponseFields]
[%autowidth, cols="m,,,,,"]

|===
3+|Field |Data Type |Size |Description
...
. -> Send an <<PPv2_CC_IPP_seamlessCustomizable_Samples_initialPaymentRequest, initial payment request>> to _{payment-gateway}_. This request must contain
  - MAID
...
. _{payment-gateway}_ returns the ``payment-redirect-url`` that contains a list of available IPP banks, programs and tenures. 
...
. -> Use the BIN to offer the appropriate installment/tenure option to your consumer **on your payment page** (not handled by iframe).

. The consumer selects an installment/tenure option and clicks "Submit/Pay" on your payment page.

. Clicking "Submit/Pay": +
  .. If the Installment Calculator response contains ``program-id``, ``plan-code`` and ``tenure``, clicking "Submit/Pay" calls a script similar to:
+
[source,js,subs=attributes+]
----
WPP.seamlessSubmit({
...
|String
|20
|Unique ID for this IPP issued by Wirecard. Maps to a specific tenure period and interest rate as agreed upon with the acquirer.
|===
+

These fields are **mandatory** in the ``extPayment`` object of the ``WPP.seamlessSubmit`` function. They specify the IPP option selected by the consumer. 
+
////
-> Retrieve the field values from the <<GeneralPlatformFeatures_IPP_InstallmentCalculator, Installment Calculator response>>.
+
//// 
.. If the Installment Calculator response contains ``installment-plan-id``, clicking "Submit/Pay" calls a script similar to:
+
[source,js,subs=attributes+]
----
WPP.seamlessSubmit({
...
----
+
``installment-plan-id`` is mandatory in the ``extPayment`` object of the ``WPP.seamlessSubmit function``. They specify the IPP option selected by the consumer.
+
//// 
-> Retrieve the ``installment-plan-id`` from the <<GeneralPlatformFeatures_IPP_InstallmentCalculator, Installment Calculator response>>.

+
//// 
The ``extPayment`` data are automatically merged with the other payment data (credit card number, payment details from the initial payment request) and sent to _{payment-gateway}_.
