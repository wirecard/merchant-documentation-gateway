...
* **standard - non-customizable:** The IPPs offered to your consumers are the same for each payment. The consumer selects from different IPP options.
...
[#PPv2_CC_IPP_seamlessStandard_Workflow]
[discrete]
===== Workflow
...
For that purpose, you have to
 ...
 If this field is missing or set to ``false``, the payment process falls back to <<PPv2_CC_IPP_seamlessStandard, non-customizable IPP>>.
...
NOTE: To retrieve all available IPP options, send a request to the <<PPv2_CC_IPP_seamlessCustomizable_Samples_IPPSRequest, Installment Payment Plan Service (IPPS)>>.

[#PPv2_CC_IPP_seamlessCustomizable_Workflow]
[discrete]
===== Workflow

. -> Send an IPPS (Installment Payment Plan Service) request to retrieve the available IPPs:
+
[#PPv2_CC_IPP_seamlessCustomizable_Samples_IPPSRequest]
.Sample IPPS Request
Endpoint: \http://d-ipps-app01.wirecard.sys:9000/v2/ +
Authentication:
+
[source,js,subs=attributes+]
----
...
. IPPS returns a list of available IPP programs and tenures:
+
...
. -> Send an <<PPv2_CC_IPP_seamlessCustomizable_Samples_initialPaymentRequest, initial payment request>> to WPG. This request must contain
  - MAID
...
+
-> Retrieve the field values from the <<PPv2_CC_IPP_seamlessCustomizable_Samples_IPPSResponse, IPPS response>>.
+
