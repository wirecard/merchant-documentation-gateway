...
[#3DS2_worfklow_basic]
=== Basic Worfklow
The 3D Secure 2 worflow remains identical to the 3D Secure 1 workflow. Find a schematic diagram of the basic workflow below:

image::images/16-05-appendix-f/3Dsecureprocessnew.png[3D Secure Workflow, align="center"]

NOTE: A Message Passing Interface (MPI) allows you to communicate with the card scheme's directory servers. The MPI can be integrated in your website or hosted by a service provider or an acquirer. Both options, an integrated or hosted MPI, allow for the enrollment check and payment authentication.
