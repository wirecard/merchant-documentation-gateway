...
The 3D Secure 2 worflow remains identical to the 3D Secure 1 workflow. Find a schematic diagram below:
...
NOTE: Sending specific 3D Secure 2 fields in the initial payment request reduces the likelihood of an authentication challenge. For more details on such fields, go to our <<3DS2_Fields, 3D Secure field table>>.
...
ifndef::env-nova[]
*1.2* Initiate the payment session with {payment-gateway} using the check-enrollment transaction type. Provide additional fields for 3D Secure 2 transactions. 3D Secure 2 fields can be found in the <<3DS2_Fields, 3D Secure field table section>> and are included in the <<Appendix_Xml, REST API payment XSD>>. +
endif::[]

ifdef::env-nova[]
*1.2* Initiate the payment session with {payment-gateway} using the check-enrollment transaction type. Provide additional fields for 3D Secure 2 transactions. 3D Secure 2 fields can be found in the <<3DS2_Fields, 3D Secure field table section>>. +
endif::[]
...
ifdef::env-nova[]
*1.2* Initiate the payment session with {payment-gateway} using the check-enrollment transaction type. Provide additional fields for 3D Secure 2 transactions. 3D Secure 2 fields can be found in the <<3DS2_Fields, 3D Secure 2 field table section>>. +
endif::[]
...
ifndef::env-nova[]
*1.2* Initiate the payment session with {payment-gateway} using the check-enrollment transaction type. Provide additional fields for 3D Secure 2 transactions. 3D Secure 2 fields can be found in the <<3DS2_Fields, 3D Secure 2 field table section>> and are included in the <<Appendix_Xml, REST API payment XSD>>. +
endif::[]
