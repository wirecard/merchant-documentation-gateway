...
[NOTE] 
====
The <<3DS2_Worfklow, 3D Secure 2 workflow>> remains identical to the 3D Secure 1 workflow.
====
...
Detailed payment flows of typical 3D Secure 2 use cases can be found <<CreditCard_3DS2_UseCases, here>>. More information on the general 3D Secure 2 worflows is given in the <<3DS2_Worfklow, 3D Secure 2 Workflows section>>.
...
|===
s|Get the 3D Secure 2 postman collection for a one-time payment transaction: a|pass:[<a href="resources/3-d-secure-2/usecase_pm-collection/one-time-payment-option-one.postman_collection.json" target="_blank" rel="noreferrer noopener" download>Download</a>]
|===
...
[NOTE]
====
More details on 3D Secure fields included in the sample requests can be found in the <<CreditCard_3DS2_Fields, 3D Secure 2 field table>>. 
ifdef::env-wirecard[]
ifndef::env-nova[]
...
. Click on the arrow next to the collection name.
