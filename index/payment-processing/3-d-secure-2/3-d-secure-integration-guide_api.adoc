...
NOTE: Download a <<3DS2_Testing_Samples, 3D Secure 2 postman collection for all use cases>>.
...
[NOTE]
====
For more details on 3D Secure fields included in the sample requests, go to our <<3DS2_Fields, 3D Secure 2 Fields section>>. 
ifdef::env-wirecard[]
ifndef::env-nova[]
...
To integrate Webhook.site in a test setup:
...
. Copy paste the HTTP POST request in the field *Response Body*. 
+
