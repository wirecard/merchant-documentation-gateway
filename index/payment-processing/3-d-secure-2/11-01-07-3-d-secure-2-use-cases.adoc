...
.Option 1: {payment-provider-name} 3DS server and external acquirer

The merchant does 3DS Authentication with {payment-provider-name} but _authorization_ with an external provider (acquirer).
...
+
The merchant receives the following values after a successful _check-payer-response_: 
+
...
Forward these values to your _authorization_ provider (acquirer).

.Option 2: Non-{payment-provider-name} 3DS server and {payment-provider-name} acquirer

The merchant receives the 3DS Authentication from a non-{payment-provider-name} 3DS server and proceeds with {payment-provider-name} for _authorization_.

The merchant receives the following authentication values from the external 3D Secure provider (e.g. the agent):
