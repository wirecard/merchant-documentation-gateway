...
[#CreditCard_3DS2_UseCase_AgentModel_ThirdPartyMPI]
=== External 3D Secure Providers
*You are a merchant with {payment-provider-name}.*

You receive the following 3D Secure 2 values from your external 3D Secure provider (a third-party MPI rather than {payment-provider-name} MPI) after a successful authentication. You need these values to proceed with the checkout in ``purchase``, ``authorization`` or ``authorization-only`` transactions:
