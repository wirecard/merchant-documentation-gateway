...
One-time payment is one of the most common scenarios: a consumer places an order for a product or service. The merchant fulfills the order in a single shipment and before expiration of the _authorization_.
...
include::include/3DSecure/3-d-secure-2-authentication-transactions.adoc[leveloffset=+2]

include::include/3DSecure/3-d-secure-2-purchase-transaction.adoc[leveloffset=+2]
...
include::include/3DSecure/3-d-secure-2-authorization-transaction.adoc[leveloffset=+2]

include::include/3DSecure/3-d-secure-2-capture-authorization-transaction.adoc[leveloffset=+2]
...
the payment session. New 3D Secure 2 fields can be found in the <<CreditCard_3DS2_Fields, 3D Secure 2 field table>>.
ifdef::env-wirecard[]
They are also included in the <<Appendix_Xml, REST API payment XSD>>.
endif::[]
+
