...
The check-enrollment is not required prior to this request, as merchant-initiated transactions are out of scope for SCA. To mark this transaction as merchant-initiated, set the request ``authorization-reason`` field to ``reauthorization``.
