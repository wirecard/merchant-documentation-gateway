...
For card-on-file transactions, merchants store the consumer's card details (as a token) for future transactions. Thus, when the consumer returns to the merchant's shop, they do not need to re-enter their data.
...
=== Option 1: Storing Card Details - Independent of Purchase
...
=== Option 2: Storing Card Details - During Purchase
