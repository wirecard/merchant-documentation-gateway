...
// vhauss: Do we have a style convention for field elements? Here you do it like ``<periodic-type>recurring</periodic-type>`` in other sections we go like ``periodic-type`` = ``recurring``. 
// vhauss: If we don't have a style guide for this, this should be the moment to create one. (-:
If the request is successful, the response contains a ``transaction-id``.
// vhauss: The transaction response always (successful or fail) contains a ``transaction-id`` 
...
// vhauss: every other transaction step we start with "Send a request with transaction type..." Why not here?
The request must contain ``<periodic-type>recurring</periodic-type>`` and ``<sequence-type>recurring</sequence-type>``. +
// vhauss: see comment above: "vhauss: Do we have a style convention..."
As ``parent-transaction-id``, enter the ``transaction-id`` from the *first* authorization response (see step 4).
// vhauss: something to think about: this numbering can be kind of tricky. Here we use a "hard" numbering while above we use a dynamic numbering. If we insert an additional step above we have to make sure to remember this down here to adjust the number accordingly. Alternative: (see step *Authorization*).
...
*or*
// vhauss: for conformity: shouldn't we add an "either" underneath the headline "For each subsequent recurring payment..." to make clear that these two blocks can be used optionally? That makes it clear from the beginning that the reader has two alternative entry points. This brings it much closer to loops in programing (e.g. "if loops").
[start=6]
...
// vhauss: all my comments above ("Option 1: Payment Due at Sign-up") apply here as well.
