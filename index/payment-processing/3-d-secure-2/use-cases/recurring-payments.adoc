...
. The first payment is consumer-initiated and therefore requires Strong Customer Authentication. This first payment can also be a zero amount without actual funds transfer.
. All succeeding payments are merchant-initiated transactions that do not need SCA.
...
.For each subsequent recurring payment (MIT)
[start=6]
. *Authorization* (subscription fee) as MIT +
...
. *Capture* or *Purchase* (subscription fee) +
...
The amount in the capture/purchase request must not exceed the authorized amount.
