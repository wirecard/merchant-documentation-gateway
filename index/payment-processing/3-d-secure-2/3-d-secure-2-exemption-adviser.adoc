...
Since 14 September 2019, the PSD 2 Regulatory Technical Standard requires SCA for 3D Secure 2 payment processes. This also applies to recurring payments (see Article 14 RTS). From that day on, each first transaction of a recurring merchant-initiated transaction (MIT) must meet the standards of PSD 2.
...
If your acquirer is Wirecard, the Wirecard Payment Gateway can "grandfather" a recurring MIT if you provide the referencing transaction-id from the first authorization or purchase transaction.
...
| First Transaction | Before 14 September 2019 | ``authorization`` & ``capture`` or +
                                                 ``purchase`` | e.g. 12345 | ``first`` | ``recurring`` or +
                                                                                         ``installment`` or +
                                                                                         ``UCOF`` | none
| Recurring Transactions | After 14 September 2019 | ``authorization`` & ``capture`` or +
                                                     ``referenced-authorization`` & ``capture`` or +
                                                     ``referenced-purchase`` | e.g. 56789 | ``recurring`` or +
                                                                                            ``final`` | ``recurring`` or +
                                                                                                        ``installment`` or +
                                                                                                        ``UCOF`` +
