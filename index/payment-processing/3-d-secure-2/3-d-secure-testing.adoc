...
NOTE: Use this test credentials to test 3D Secure transactions for both versions, 3D Secure 2 as well as 3D Secure 1.
...
For <<3DS2_IntegrationGuide, 3D Secure transactions>>, both 3D Secure 2 and 3D Secure 1,
{payment-gateway-abbr} checks with the first request, whether the card is enrolled in the 3D Secure program.
...
|===
s|Get the 3D Secure 2 Postman Collection: a|pass:[<a href="resources/3-d-secure-2/usecase_pm-collection.zip" target="_blank" rel="noreferrer noopener" download>Download</a>]
|===


The 3D Secure 2 postman collection contains sample requests for all typical use cases documented in our <<CreditCard_3DS2_UseCases, 3D Secure 2 Use Cases section>>.

Use https://www.postman.com/postman[Postman] to run the collection. Our samples work with https://webhook.site[Webhook.site] to simulate the ACS URL where the consumer will be redirected to for authentication. 

Need more information on how to use Postman to test our samples or to integrate Webhook.site in your own test setup? Go to our <<3DS2_IntegrationGuide_REST_PostmanCollection, General Integration Guide page for REST API>>.

NOTE: The postman collection is preconfigured with 3D (Manual Card Brand Recognition)test credentials and uses a Mastercard with the _Card Number_ ``5413330300201218``, _Expiration Date_ ``{cc-exp-month}/{cc-exp-year}`` and _CVC_ ``218``.
