...
The 3D Secure 2 postman collection  for all use cases contains sample requests for all typical use cases documented in our <<CreditCard_3DS2_UseCases, 3D Secure 2 Use Cases section>>.

Use https://www.postman.com/postman[Postman] to run the collection. Our samples work with https://webhook.site[Webhook.site] to simulate the ACS URL where the consumer will be redirected to for authentication. This allows you to obtain the PARes. 

Need more information on how to use Postman to test our samples or to integrate Webhook.site in a test setup? Go to our <<3DS2_IntegrationGuide_REST_PostmanCollection, General Integration Guide for REST API>>.
