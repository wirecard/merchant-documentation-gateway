...
|<<WirecardVoucher_Samples_enrollment, _enrollment_ samples>>

| activation 
| Activates an existing voucher. 
| <<WirecardVoucher_Samples_activation, _activation_ samples>>
...
If redemption was already confirmed, use transaction type _cancel-redeem_. 
|
...
| Unique code for payment and voucher processing. +
 Returned in _enrollment_ response for further use. +

Mandatory for _authorization-only_.

Mandatory <<WirecardVoucher_Fields_Footnote, *[navy]#^1^#*>> for
...
| String
| 255  
| Token as returned from ``REDEMPTION_RESERVATION``.

| voucher-description 
| O            
| String
| 255  
| Used as operational comment.
...
| String
|   
| ID of voucher type that voucher will be created from. +
