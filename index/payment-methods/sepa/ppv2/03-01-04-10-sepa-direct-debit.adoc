...
The Single Euro Payments Area (SEPA) is an initiative of the European banking industry
...
before it transfers money from the consumer account to you.
...
2+| creditor-id | String | Mandatory | 35 | This is your Creditor ID of.
|===
...
2+| request-id    | String | A unique identifier assigned to every request which you sent out. Used when searching for or referencing it later.
...
//-
  ...
  | merchant-account-id e| value | String | A unique identifier assigned to every _Merchant Account_ (by {payment-provider-name}).
...
2+| creditor-id | String | This is your Creditor ID.
|===
...
2+| iban          | String | The International Bank Account Number (IBAN).
2+| request-id    | String | A unique identifier assigned to every request which you sent out. Used when
