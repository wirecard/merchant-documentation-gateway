...
ifdef::env-nova[]
2+h| URL (Endpoint) |``\https://payments-test.wirecard.com/ee/rest/payments``
endif::[]
...
|device/type                            |M |O |  |Enum    |   |Type of device which submits the RTP. Enum types: mobile, pc, tablet, other.
|device/operating-system                |M |O |  |Enum    |   |Operating system used on the device. Enum types: android, ios, windows, windows-mobile, other.
