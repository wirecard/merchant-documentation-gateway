...
//-
For ``klarna-debit``, ``klarna-transfer`` and ``klarna-paylater``, Klarna supports <<KlarnaV2_TransactionTypes_authorizationRecurring, recurring>> payments.
...
For the initial authorization of recurring payments: +
  ...
  ** transaction type ``authorization`` **and**
  ** sequence-type ``first``
//-
 ...
 * transaction type ``authorization`` **and**
 * sequence-type ``recurring`` or ``final``
...
<<KlarnaV2_TransactionTypes_authorizationRecurring_Samples, recurring samples>>
