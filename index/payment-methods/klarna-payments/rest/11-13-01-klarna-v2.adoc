...
{payment-gateway} processes reconciliations with two internal transaction types:
...
| revoke-capture	
| You deposit funds: +
 In disputed orders, _Klarna_ registers this refund transaction in favour of the consumer. +
 This contains fraudulent cases seen as merchant loss, eg. due to missing tracking data.

| adjustment-cr
| You get credit: +
Due to _Klarna’s_ merchant protection (eg. in case of fraud), _Klarna_ credits the net-amount of the order value (without VAT).
