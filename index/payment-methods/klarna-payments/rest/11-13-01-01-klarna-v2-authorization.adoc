...
|Boolean
|
a|``true``: If this ``authorization`` is successful, it will be <<KlarnaV2_TransactionTypes_authorization_autoCapture, captured automatically.>> +
...
* A <<KlarnaV2_TransactionTypes_captureAuthorization, ``capture-authorization``>> followes automatically straight after a successful ``authorization``.
* You can also auto-capture <<larnaV2_TransactionTypes_authorizationRecurring, recurring payments>>.
...
The <<KlarnaV2_TransactionTypes_authorization_Fields_orderItem_type, ``order-item.type``>> of each ``order-item`` must be <<KlarnaV2_TransactionTypes_authorization_Fields_orderItem_type_digital, ``digital``.>> +
