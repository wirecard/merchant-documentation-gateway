...
|get-url
|<<API_AlipayQRPay_Samples_geturl, _debit/get-url_ sample (REST API)>>
...
|refund-debit | <<API_AlipayQRPay_Samples_refunddebit, _refund-debit_ sample>>
...
<<API_AlipayQRPay_Samples_VoidDebit, _void-debit_ sample>>
|===
...
image::images/11-49-alipay-qr-pay/AlipayQRWorkflow.png[Alipay QR Pay Workflow]

. Consumers approach the cashier to pay for the selected items.
. The merchants create a request to the {payment-gateway} to generate the QR code so that the consumers can make the payment.
...
The fields are ordered hierarchically and in the follwing way: +
 ``payment`` is the top parent element. +
 ``custom-fields``, ``wallet`` and ``payment-methods`` are children of ``payment``. +
 ``account-id`` is a child of ``wallet``. +
 ``payment-method`` is a child of ``payment-methods``. +
 ``custom-field`` is a child of ``custom-fields``. +
 ``field-name`` defines its respective parent. +
 ``field-value`` adds a description to the respective ``field-name``.
...
|Notification only
|14
|The time when the payment is completed. Can be used on receipt. +
e.g. 20200220225336
...
[#API_AlipayQRPay_Fields_payment.wallet]
==== ``wallet``
[%autowidth, cols="m,,,"," stripes=none]
|===
|Field Name|<<APIRef_FieldDefs_Cardinality, M/O>> |Size |Description
...
[#API_AlipayQRPay_Fields_payment.payment-methods]
==== ``payment-methods``
[%autowidth, cols="m,,,"," stripes=none]
|===
|Field Name|<<APIRef_FieldDefs_Cardinality, M/O>> |Size |Description
...
----
include::{root}/samples/xml/alipay-qr-pay_get-url-request_success.xml[]
----
...
[source,xml,subs=attributes+]
----
include::{root}/samples/xml/alipay-qr-pay_debit_get-url-request_success.xml[]
----

.XML Debit/Get-url Response (Successful)
[source,xml,subs=attributes+]
----
include::{root}/samples/xml/alipay-qr-pay_debit_get-url-response_success.xml[]
----
...
[source,xml,subs=attributes+]
----
include::{root}/samples/xml/alipay-qr-pay_debit_request_failure(1).xml[]
----
