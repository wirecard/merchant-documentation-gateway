...
Partial and multiple captures are allowed as long as you do not exceed authorization limit. +
| <<payolution_Samples_captureAuthorization, capture-authorization samples>>

| debit
| Authorizes and captures a payment in one go and triggers fund transfer. +
| <<payolution_Samples_debit, debit samples>>
...
For ``payolution-inst``, you can only add information to a preceding authorization request. You cannot change neither amount nor number of installments.
|
...
[#payolution_ELV]
==== Direct Debit - ``payolution-elv`` (debit)
...
As ``field-value`` for ``custom-field field-name="duration"``, enter the selected number of installments. +
If you do not provide this field, the request falls back to async installment.
...
Fields can be **M**andatory or **O**ptional in an _authorization_ request for any
...
| months
| [[payolution_Fields_Installments_months]]Number of installments. Enter this value as ``field-value`` for ``custom-field field-name="duration"``.
