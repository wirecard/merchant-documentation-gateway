...
. -> Send a ``debit`` request to _{payment-gateway}_. +
This request must contain ``payment-method@name`` = ``payolution-elv`` and the consumer's ``iban``.
. _{payment-gateway}_ processes the request and forwards it to _payolution_.
...
. _payolution_ sends a response to _{payment-gateway}_ and transfers funds to merchant account.
. _{payment-gateway}_ informs you about the transaction outcome.
...
. -> Send an ``authorization`` request to _{payment-gateway}_. +
This request must contain ``payment-method@name`` = ``payolution-b2c`` and the consumer's ``date-of-birth``.
. _{payment-gateway}_ processes the request and forwards it to _payolution_.
. _payolution_ runs risk checks and sends a response to _{payment-gateway}_.
. _{payment-gateway}_ forwards the response to you.
. -> Send a ``capture`` request to _{payment-gateway}_. +
...
. _{payment-gateway}_ processes the request and forwards it to _payolution_.
. _payolution_ captures the requested amount and sends a response to _{payment-gateway}_.
. _{payment-gateway}_ processes the response informs you about the transaction outcome.
...
. -> Send a ``claculate`` request to _{payment-gateway}_.
. _{payment-gateway}_ retrieves installment plans from _payolution_ and forwards them to you in the ``calculate`` response. +
...
. -> Send an ``authorization`` or ``debit`` request to _{payment-gateway}_: +
...
. _{payment-gateway}_ forwards the request to _payolution_.
. _payolution_ processes the request and sends the response to _{payment-gateway}_.
. _{payment-gateway}_ forwards the response to you.
. -> Inform the consumer about the transaction outcome.
...
. _{payment-gateway}_ checks if parts of this order have already been captured:
  ** If yes: _{payment-gateway}_ sends a _void-authorization_ request to _payolution_. 
  ** If no: _{payment-gateway}_ rejects the _void-authorization_ request. 
 ...
 ** If the _refund-capture_ request does not contain ``requested-amount``, _{payment-gateway}_ refunds the total order amount of the preceding _capture-authorization_ transaction.
