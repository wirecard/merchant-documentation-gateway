...
image::images/11-23-payolution/payolution_voidAuthorization.png[payolution Reversal, width=950]
...
. _{payment-gateway-abbr}_ checks if parts of this order have already been captured:
  ** If yes: _{payment-gateway-abbr}_ sends a void-authorization request to _payolution_. 
  ** If no: _{payment-gateway-abbr}_ rejects the void-authorization request. 
. _payolution_ approves or rejects the request and returns the result.
...
image::images/11-23-payolution/payolution_refundCapture.png[payolution Refund, width=950, align="right"]
...
// [tikz,payolution_workflow_refundCapture,svg,subs=attributes+]
// ----
// % Document Header
...
The order amount must be within the limits you contracted with _payolution_. The limits are set individually for each of
<<payolution_SpecificPaymentMethods, _payolution's_ specific payment method>>. If the order amount is not within the defined limits, the respective payment method is not available to the consumer.
