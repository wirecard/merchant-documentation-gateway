...
// [cols="h,,,"]
// |===
// | QR Code Creator > h| Processing Element > h| Presentation of QR Code > h| Final Step
...
. _SCB Prompt Pay_ sends a response containing the base64-encoded QR code to {payment-gateway}.
. {payment-gateway} sends you the QR code information as URL for {payment-provider-name} Hosted Payment Page (HPP) (field ``payment-method url``) and base64-encoded in the field ``qr-code``.
. Use the ``payment-method url`` and redirect the consumer to the {payment-provider-name} HPP which displays the QR code. 
. the consumer scans the QR code and processes the payment.
...
. {payment-gateway} creates a _get-url_ transaction and sends a request to _SCB Prompt Pay_. 
. _SCB Prompt Pay_ generates a special QR code for processing the payment via _SCB Prompt Pay_.
. _SCB Prompt Pay_ returns a message with the QR code in base64 format to {payment-gateway}.
. {payment-gateway} sends you the QR code information as URL for {payment-provider-name} Hosted Payment Page (HPP) (field ``payment-method url``) and base64-encoded in the field ``qr-code``.
. Decode the ``qr-code`` value and display the QR code directly on the consumer's checkout page.
...
NOTE: GETSTATUS can only be performed *before* {payment-gateway} sends the IPN. 
...
A generated QR code does not expire on Siam Commercial Bank _Prompt Pay's_ side. There is a default timeout period (30 minutes) which is configurable globally per {payment-gateway} instance or you can set it - 
