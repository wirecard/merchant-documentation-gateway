...
. You send _debit_ request to {payment-gateway} to process the payment with _SCB Prompt Pay_.
. {payment-gateway} creates a _get-url_ transaction and sends a request to _SCB Prompt Pay_ to generate a special QR code for processing the payment via _SCB Prompt Pay_.
. _SCB Prompt Pay_ returns message with QR code image in base64 format to {payment-gateway}.
. {payment-gateway} sends you the response as URL for {payment-provider-name} Hosted Payment Page (HPP) (field ``payment-method url``).
. This URL redirects the consumer to the {payment-provider-name} HPP which displays the QR code. 
. the consumer scans the QR code and processes the payment.
. _SCB Prompt Pay_ sends IPN message with transaction status to {payment-gateway}
. {payment-gateway} creates success _debit_.
. {payment-gateway} sends notification to you that _debit_ transaction was successfully created and processed.

.Consumer scans QR code on your web site

. You send _debit_ request to {payment-gateway} to process the payment with _SCB Prompt Pay_.
. {payment-gateway} creates a _get-url_ transaction and sends request to _SCB Prompt Pay_ to generate special QR code for processing the payment via _SCB Prompt Pay_.
. _SCB Prompt Pay_ returns message with QR code image in base64 format to {payment-gateway}.
. {payment-gateway} sends you the response QR code in base64 format (field ``qr-code``).
. Encode the value in ``qr-code`` in base64 format and display it directly on the consumer's checkout page.
. the consumer scans QR code. 
...
. You send a request to find out the status of the transaction.
...
. If timeout is expired, {payment-gateway} creates a failed _debit_ and sends you an IPN which contains the transaction status.
