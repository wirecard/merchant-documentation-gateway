...
. _SCB Prompt Pay_ returns a message with the QR code image in base64 format to {payment-gateway}.
...
. {payment-gateway} sends the IPN to you that the _debit_ transaction was successfully created and processed.

.Consumer scans QR code on your web site

. You send a _debit_ request to the {payment-gateway} to process the payment with _SCB Prompt Pay_.
. {payment-gateway} creates a _get-url_ transaction and sends a request to _SCB Prompt Pay_ to generate special QR code for processing the payment via _SCB Prompt Pay_.
. _SCB Prompt Pay_ returns a message with the QR code image in base64 format to {payment-gateway}.
