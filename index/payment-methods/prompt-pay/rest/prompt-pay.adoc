...
_Siam Commercial Bank Prompt Pay_ (_SCB Prompt Pay_) is a type of <<PaymentMethods_PaymentMode_OnlineBankTransfer, Online Bank Transfer>>.
...
This table illustrates how _SCB Prompt Pay_ notifications are encoded and which formats and methods can be used for requests and responses.
...
|_debit_ |Transaction created after _SCB Prompt Pay_ confirms by IPN if transaction was processed. | <<API_PromptPay_Samples_debit, _debit_ sample>>
...
// | {payment-gateway} | payment-method url | {payment-provider-name} HPP | wait for IPN from _SCB Prompt Pay_
// | Your web site | qr-code | Your web site | redirect consumer to _SCB Prompt Pay_ to process the payment
// |===
// +

.Consumer scans QR code on {payment-provider-name} Hosted Payment Page

. Send a _debit_ request to the {payment-gateway} to process the payment with _SCB Prompt Pay_.
. {payment-gateway} creates a _get-url_ transaction and sends a request to _SCB Prompt Pay_. 
. _SCB Prompt Pay_ generates a special QR code for processing the payment via _SCB Prompt Pay_.
. _SCB Prompt Pay_ sends a response containing the QR code information to {payment-gateway}.
...
.. let the consumer scan the QR code image and processes the payment with _SCB Prompt Pay_.
...
.. redirect the consumer to _SCB Prompt Pay_ to process the payment.
. _SCB Prompt Pay_ sends the IPN with the transaction status to {payment-gateway}.
...
// . You send a _debit_ request to the {payment-gateway} to process the payment with _SCB Prompt Pay_.
// . {payment-gateway} creates a _get-url_ transaction and sends a request to _SCB Prompt Pay_. 
// . _SCB Prompt Pay_ generates a special QR code for processing the payment via _SCB Prompt Pay_.
// . _SCB Prompt Pay_ sends a response containing the QR code information to {payment-gateway}.
// . {payment-gateway} sends you the QR code information 
...
A generated QR code does not expire on _SCB Prompt Pay's_ side. There is a default timeout period (30 minutes) which is configurable globally per {payment-gateway} instance or you can set it - 
...
All fields used for _SCB Prompt Pay_ are the same as the <<RestApi_Fields, ReST API Fields>>. Please refer to the API field list. 
