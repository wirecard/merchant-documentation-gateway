...
<<CC_Fields_xmlelements_request_payment, payment>>. ``sub-merchant-info`` fields are required for the <<CreditCard_PaymentFeatures_PaymentFacilitator, _Payment Facilitator_>>. If you want to use ``sub-merchant-info``, all the fields in this table are mandatory in the first request in the transaction flow. We recommend to include ``sub-merchant-info`` in the whole payment process. Otherwise, some transactions cannot be completed successfully. ``state`` is an exception. See ``state`` description for details.
