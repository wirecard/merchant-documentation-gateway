...
==== Additional Business Type Fields

Alipay requires information on the business type of a payment transaction. Therefore, the field ``business_type`` is mandatory in an Alipay Cross-border request. It specifies one of 5 business types:

. <<API_AlipayCrossBorder_Fields_BusinessType1, Hotel industry>>
. <<API_AlipayCrossBorder_Fields_BusinessType2, Airlines industry>>
. <<API_AlipayCrossBorder_Fields_BusinessType3, Education institutions>>
. <<API_AlipayCrossBorder_Fields_BusinessType4, Sales of goods>>
...
5+| [[API_AlipayCrossBorder_Fields_BusinessType1]]
 **Business Type 1 - Hotel Industry** +
Each field is a child of ``hotel-industry`` and mandatory if ``business_type``&nbsp;=&nbsp;``1``. 
...
5+| [[API_AlipayCrossBorder_Fields_BusinessType2]] **Business Type 2 - Airlines** +
Each field is a child of ``airline-industry.itinerary.segment`` and mandatory if ``business_type``&nbsp;=&nbsp;``2``. 
...
| 
| Flight number. +
Mandatory if ``business_type`` is ``2``. +
...
5+| [[API_AlipayCrossBorder_Fields_BusinessType3]] **Business Type 3 - Education Institutions** +
...
[NOTE]
{payment-provider-name} does not validate this field but rather forwards it to Alipay.

5+| [[API_AlipayCrossBorder_Fields_BusinessType4]] **Business Type 4 - Sales of Goods** +
Each field is a child of ``order-items.order-item`` and mandatory if ``business_type``&nbsp;=&nbsp;``4``. 
...
// A concatenation of order_item.name^order_item.quantity|order_item.name^order_item.quantity

// goods_info	String


// Calculation needed	 	Calculated from Sum of all Amounts in order_item.quantity	total_quantity	Number

5+| [[API_AlipayCrossBorder_Fields_BusinessType5]] **Business Type 5 - Other** +
The business type description of your business is stored in the MCC code of your merchant configutation. If ``business_type``&nbsp;=&nbsp;``5``, {payment-gateway} adds the MCC code description to your request. +
For setup, <<ContactUs, contact merchant support>>.

|===


[#API_AlipayCrossBorder_Fields_autoDebit]
==== Additional _auto-debit_ Fields
