...
Alipay requires information on the business type of a payment transaction. Therefore, {payment-gateway} adds the ``business_type`` as specified in your merchant configuration to each Alipay Cross-border request. +
...
Each business type has additional mandatory fields to be sent wich each request.
