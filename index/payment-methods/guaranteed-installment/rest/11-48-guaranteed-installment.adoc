...
at <<AppendixB, API Reference: Transaction Types>>.
...
Consumer->>Consumer: selects _{payment-provider-name} Payment in Installments_
...
. Consumer selects the payment method _{payment-provider-name} Payment in Installments._
...
|String
|30
|This is the type for a transaction: _authorization_, _capture_, _void-authorization_, _refund-capture_ and _credit_ are supported for _{payment-provider-name} Payment in Installments_.
...
|_currency_ (attribute of ``requested-amount``)
|M
|String
|3
...
|[[GuaranteedInstallment_Fields_PaymentMethods]]_name_ (attribute of ``payment-method``)
|M
|String
|15
...
2+| With these fields you receive the configuration available with _Guaranteed_. With this information you can provide the installment options you can present to your consumers.
|interest-rate-default|Decimal|The default interest-rate will be used if submitted interest-rate from the calculation request deviates from the range of your supported rates. (Example: ``13.7``).
...
_{payment-provider-name} Payment in Installments_ supports two different installment plans:
...
|amount|M|Decimal| 18.3 |Full amount of the purchase. Must be in the format of _Money_ (Example: ``100.00``)
...
|rate|M/O|Decimal| 3.2 |Monthly rate which the consumer wants to pay. Must be in the format of _Money_ (Example: ``25.00``). It is *mandatory* for  for calculation by rate. It must not be used when calculating the installment plan by time.
