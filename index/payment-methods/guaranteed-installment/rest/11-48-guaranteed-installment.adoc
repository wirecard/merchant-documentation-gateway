...
|authorization    | Performs a creditworthiness check at the moment when 
                    ...
                    the basket.
                                | <<GuaranteedInstallment_Samples_Authorization, authorization samples>>
...
|capture-authorization | Used to capture the reserved funds at the moment
                         ...
                         amount of the capture must be the value of the items in the basket.
                                | <<GuaranteedInstallment_Samples_CaptureAuthorization, capture-authorization samples>>
...
|void-authorization | Used when the consumer wishes to cancel some or all
                      ...
                      or shipped. Basket items only list those items which are canceled from the order.
                                | <<GuaranteedInstallment_Samples_VoidAuthorization, void-authorization samples>>
...
|refund-capture | Used when the consumer wishes to return some or all
                  ...
                  been returned. Basket items list only those items which are returned to the merchant.
                                | <<GuaranteedInstallment_Samples_RefundCapture, refund-capture samples>>
...
|credit | Used when the merchant wishes to grant a discount on an existing order. Items from this order may already be delivered to the consumer.
          ...
          to the successful authorization ``transaction-id``.
                                | <<GuaranteedInstallment_Samples_Credit, credit samples>>
|===
...
You can use these credentials - aside from the endpoints - for payment tests as well as testing the <<GuaranteedInstallment_Config, installment configuration service>> and the <<GuaranteedInstallment_Features_InstallCalc, installment calculator>>.
...
. Merchant sends an authorization request including basket items to {payment-gateway-abbr}.
. {payment-gateway-abbr} generates a descriptor (an alphanumerical code, starting with DG).
. {payment-gateway-abbr} sends authorization response to merchant (the authorization response includes the descriptor).
. Merchant sends a capture request to {payment-gateway-abbr} when the basket items are ready for delivery.
. After a successful capture request, {payment-gateway-abbr} generates the invoice, including the descriptor. +
.. Alternatively, merchant generates the invoice. 
...
*C* for sending a request for _{payment-provider-name} Payment in Installments_ (complete field list available in <<RestApi_Fields, REST API field list>>):
...
|currency (attribute of requested-amount)
|M
|String
|3
...
|[[GuaranteedInstallment_Fields_PaymentMethods]]name (attribute of payment-method)
|M
|String
|15
...
|[[GuaranteedInstallment_Fields_ShippingAaddress]]shipping.address fields have to match the billing address (specified as <<GuaranteedInstallment_Fields_AccountHolderAddress, account-holder.address>>).
...
2+h| Please use these <<GuaranteedInstallment_TestCredentials, test credentials>>.
|===
...
|description (attribute of status)|String|The description of the response status code (Example: ``Successful Response``).
|severity (attribute of status)|String|This field indicates if a status is an error or information. Values can be: +
``information`` +
``warning``.
...
|currency (attribute of amount)|M|String|3|The currency of the purchase. _{payment-provider-name} Payment in Installments_ requires EUR.
...
|currency (attribute of rate)|M|String|3|The currency of the rate. _{payment-provider-name} Payment in Installments_ requires EUR.
...
|currency (attribute of amount)|String|The currency of the purchase. _{payment-provider-name} Payment in Installments_ requires EUR.
|rate|Decimal|Monthly rate which the consumer wants to pay. It must be in the format of _Money_ (Example: ``25.00``).
|currency (attribute of rate)|String|The currency of  the rate. _{payment-provider-name} Payment in Installments_ requires EUR.
...
|code (attribute of status)|String|The status code of the response (Example: ``200.0000``).
|description (attribute of status)|String |The description of the response status code (Example: ``Successful Response``).
|severity (attribute of status)|String|This field indicates the status of the transaction. Values can be: +
``information`` +
``error``.
...
Basket items details must be <<GuaranteedInstallment_Fields_OrderItems, sent with every request>>. Content of
