...
Please be aware that your customers will only be able to see the _Apple Pay_ payment method if they:
...
|In order to set up _Apple Pay_ on your webpage, you need a Merchant Identity Certificate. A Merchant Identity Certificate is a Transport Layer Security (TLS) certificate associated with your Apple Merchant ID, used to authenticate your sessions with the _Apple Pay_ servers. The merchant identity certificate is only required for _Apple Pay_ on the web; it is not needed to accept payments in your apps.

|Payment Sheet
|The _Apple Pay_ Payment sheet is the screen that consumers will see when they initiate an _Apple Pay_ payment. It can include the user's name, billing address, shipping address, shipping method, phone number, and email address needed to complete the transaction.
|===
...
* To ensure a unique merchant identifier we recommend to use the prefix: *merchant.com.wd*.
+
For example: *merchant.com.wd.merchantID*.

. Review the settings, then click *Register*.
. When signing your _Apple Pay_ contract, please provide this Merchant Identifier to your key account manager. {payment-provider-name} Merchant Support then sends you an email with your {payment-provider-name} account details and a {payment-provider-name} Certificate Signing Request (CSR) file. 

//-

If you create your Merchant Identifier after signing your contract, please follow the above instructions, then email the Merchant Identifier directly to {payment-provider-name} mailto:support@wirecard.com?subject=Apple%20Pay%20Certificate%20Signing%20Request[Merchant Support] and request a CSR file.
...
* When you are prompted to upload a certificate signing request, upload the {payment-provider-name} CSR file you received from the {payment-provider-name} Merchant Support.
. Download and save the Payment Processing Certificate (a file with a ``.cer`` extension).
+
NOTE: Once created, _Apple Pay_ can encrypt your payment requests and {payment-provider-name} can validate the payments. No further steps are required regarding the Payment Processing Certificate. +
...
. Follow https://help.apple.com/developer-account/#/dev4cb6dfbdb?sub=dev3f2891710[Apple's instructions] for enabling _Apple Pay_.
...
* When entering your domain names, both top-level (e.g. http://wirecard.com/[{domain}]) and all subdomains where the _Apple Pay_ logo will be displayed (e.g. http://wirecard.com/[{documentation-url}]) must be registered. 
+
NOTE: You can register multiple domains under the same Merchant ID. This is often necessary if you use different domains in various geographies. +
...
====
There is no public merchant-account available. Apple Pay can be enabled provided that you have a CSR and an Apple Developer Account. 
====
...
. Your webpage sends a request/queries the consumer’s device to check if _Apple Pay_ is available (installed with a card loaded into the wallet).
. The device returns a confirmation response to your webpage.
. The webpage displays the *Apple Pay* button.
. The consumer clicks the *Apple pay* button and the Apple Pay Payment Sheet is displayed.
. Your webpage starts a payment session with your server that hosts the webpage.
. Your server starts a payment session with the Apple servers.
. The Apple servers return session information to your servers.
. Your server returns session information to the consumer-facing webpage.
. The webpage authenticates/validates you to the consumer’s iOS/MacOS device using the session information.
. The payment refreshes and displays the consumer’s preferred address, contact info, and shipping information retrieved from the consumer’s Apple wallet.
. The consumer authorizes the transaction with Touch ID (or Face ID).
. Touch ID (Face ID) is a match and the payment is authorized.
. The consumer’s device communicates with the Apple servers to exchange nonces.
. The consumer’s device enables/activates the secure element (a secure piece of hardware in Apple devices which stores the consumer's payment data and cannot be duplicated or in any way tampered with).
. The secure element generates a cryptogram.
. The secure element encrypts the consumer’s payment data with Apple Keys.
. The paymentData object (in Apple encryption) is routed to the Apple servers.
. The Apple servers unwrap the paymendData object with your public key.
. Apple servers send paymendData object (in merchant encryption) to the consumer’s iOS/MacOS device
. The consumer’s billing/shipping/contact data is sent to the merchant’s webpage via API callback
. The billing/shipping/contact data are passed to the merchant’s server
. The merchant’s server decrypts the paymendData object to retrieve the consumer’s DPAN, expiration date of card, and cryptogram
. The merchant’s server sends this decrypted data to Wirecard for processing/authorization
. Wirecard sends the data to the card networks using 3DS message format
. The card networks send the data to the issuing bank of the consumer for authorization
. The issuing bank authorizes the payment
. The payment sheet is dismissed and the purchase confirmation page is displayed on the consumer’s iOS/MacOS device
...
In-store, _Apple Pay_ functions as a NFC-capable credit card, but gets customers through the checkout more quickly. _Apple Pay_ especially improves the customer experience online. It can eliminate the need for customers to create accounts or fill out lengthy forms when making purchases on the web in Safari on their iPhone, iPad, or Mac, thereby eliminating two of the main causes of cart abandonment.

|Is _Apple Pay_ secure?
2+|Tokenization and two-factor authorization, requiring a password or authentication through Face ID or Touch ID mean _Apple Pay_ is one of the most secure payment methods available. Tokenization means the customer’s payment details are not stored in the Apple Wallet, rather a randomized number (token) is stored instead.
...
2+|No. _Apple Pay_ is processed at the same rate as the credit or debit card your customer has in the Apple Wallet.
...
|How do I let my customers know that they can pay with _Apple Pay_?
2+a|. Display the https://support.apple.com/kb/HT208087[payment symbols for your region and the _Apple Pay_ mark] prominently on your point-of-sale terminal to let your customers know they can use _Apple Pay_ in your store. +
...
2+|Check the http://www.apple.com/apple-pay[_Apple Pay_ webpage] or https://getsupport.apple.com/?caller=psp&PGF=PGF90001[contact merchant support].
|===
...
|What customer information can I pull from _Apple Pay_?
2+|Customer information includes a phone number and billing, shipping, and email addresses. For more details, see https://developer.apple.com/documentation/passkit/pkcontactfield[PKContactField].

|Where does information come from in the payment sheet?
2+|The information comes from Wallet & Apple Pay defaults in *Settings*.

|Is the customer information that comes from _Apple Pay_ verified by Apple?
2+|Customer information is shared as-is, and is not verified or standardized by Apple. You need to validate it on your platform and communicate through the _Apple Pay_ API if fields should be corrected.

|What does the _Apple Pay_ payment token contain?
2+|The structure, format, and data included in the PKPaymentToken can be found in the https://developer.apple.com/library/archive/documentation/PassKit/Reference/PaymentTokenJSON/PaymentTokenJSON.html[Payment Token Format Reference]. PKPaymentTokens only contains information on processing the payment; the consumer information is included in the https://developer.apple.com/documentation/passkit/pkpayment[PKPayment] object. The PKPayment encapsulates the consumer information and the PKPaymentToken.
...
To see the last four digits of the Device Account Number, ask the customer to go to Wallet, tap the card, and tap on the top-right corner of the display.
You can also have the customer hold their iPhone or Apple Watch near the reader, select the card that they used to make the original payment, and authorize the return with https://support.apple.com/kb/HT208109[Face ID], Touch ID, or a passcode.

|Is there a limit to the transaction amount that customers can use _Apple Pay_ for in my store?
2+|_Apple Pay_ allows your customers to make easy and secure contactless payments of any amount. Read about https://support.apple.com/kb/HT207435[transaction limits].
|===
