...
. The payment refreshes and displays the consumer’s preferred address, contact, and shipping information retrieved from the consumer’s Apple wallet.
...
. The Apple servers send paymendData object (in your encryption) to the consumer’s iOS/MacOS device.
. The consumer’s billing, shipping, and contact data is sent to your webpage via API callback.
. The billing, shipping, and contact data is sent to your server.
. Your server decrypts the paymendData object to retrieve the consumer’s DPAN, card expiration date, and cryptogram.
. Your server sends this decrypted data to {payment-provider-name} for processing and authorization.
. {payment-provider-name} sends the data to the card networks using the 3DS message format.
