...
. Your webpage queries the consumer’s device to check if _Apple Pay_ is available (installed with a card loaded into the wallet).
. The device returns a confirmation response to your webpage.
. Your webpage displays the *Apple Pay* button.
. The consumer clicks the *Apple pay* button and the _Apple Pay_ Payment Sheet is displayed.
. Your webpage starts a payment session with the server that hosts the webpage.
...
. The webpage validates you to the consumer’s iOS/MacOS device using the session information.
...
. The consumer’s device enables the secure element (a secure piece of hardware in Apple devices which stores the consumer's payment data and cannot be duplicated or in any way tampered with).
...
. The consumer’s billing, shipping, and contact data is sent to your webpage through API callback.
