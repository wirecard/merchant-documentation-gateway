...
{payment-provider-name} lets you take advantage of fast, cashless payments for
...
|{payment-provider-name-lower-case}
|Secret Key
...
.Testing with a non-tokenized card (Google Pay™ cryptogram contains the PAN)
...
. Modify to use '{payment-provider-name-lower-case}' as the GATEWAY_TOKENIZATION_NAME and the provided merchant-account-id as gatewayMerchantId in GATEWAY_TOKENIZATION_PARAMETERS.
. Google Pay™ returns a payment token response in JSON format:
+
[source,json,subs=attributes+]
----
{
   "signature":"MEUCIBGhHfITKGeG8zVTiAKhpJU1Syvv2M3B7DkJe3WnFKo0AiEAtJ3z0avPG/f/jFxEMElU2Pr1CQN6Vm0evlVBw0do6mk\u003d",
   "protocolVersion":"ECv1",
   "signedMessage":"{\"encryptedMessage\":\"\\u003d\\u003d\",\"ephemeralPublicKey\":\"BHLFVI1pV4zGesdvj7BR1jOGP3c266Rm1e7N0a9teNnLmfzCUp2MuJqQD4RbrsSIotpIKWdKtVmwb5AdHFwoGhs\\u003d\",\"tag\":\"yl7NXGb3RDLvllAhDos37xd9E82+pdzfYFV1o2Rz0fQ\\u003d\"}"
}
----
+
. Create an ``authorization`` or ``purchase`` request. This request must contain the "encryptedMessage" of Google Pay™'s payment token response in the field ``cryptogram-value``. 
. Send this request to {payment-gateway} for processing. For details see the <<GooglePay_Samples_Authorization, ``authorization``>> or <<GooglePay_Samples_Purchase, ``purchase``>> request containing the Google Pay™ cryptogram.
...
. Follow Google Pay™ instruction to create a test app to generate a test cryptogram. +
- For in-app integration, please refer to +
https://developers.google.com/pay/api/android/guides/setup
& +
https://developers.google.com/pay/api/android/guides/tutorial. +
