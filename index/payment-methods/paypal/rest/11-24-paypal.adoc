...
| locale | O | O | O | String | 6 | Code of the language in which {payment-gateway-abbr} renders the _PayPal_ page. Typically used in conjunction with Hosted Payment Page. +
Format can be either ``language`` (e.g. ``EN``) or ``language_country`` (e.g. ``EN_US``).
