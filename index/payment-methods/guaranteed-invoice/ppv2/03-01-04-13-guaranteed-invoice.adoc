...
With Guaranteed Invoice, {payment-provider-name} guarantees that merchants receive the payment amount, even if the consumer does not pay.
...
[#PPv2_GuaranteedInvoice_GeneralInformation]
=== General Information
...
include::/{base_dir}/include/ppv2-general-information-2.adoc[]
...
[#PPv2_GuaranteedInvoice_Countries]
...
[WARNING]
====
To get the approval to go live, every merchant needs to complete a number of <<GuaranteedInvoice_ProcessChain, process chains>> in a test environment successfully. {payment-provider-name} checks and approves the frontend/checkout and invoices (if generated and sent out by the merchant).
====

[#PPv2_GuaranteedInvoice_TransactionType_authorization]
=== Transaction Type _authorization_

An _authorization_ reserves the specified amount from the account holder’s bank account for a later transfer. For Guaranteed Invoive, it also triggers a creditworthiness-check.
...
----
include::/{base_dir}/samples/json/PPv2GuaranteedInvoiceTransactionTypeAuthorizationRequest.json[]
----
NOTE:  <<PPv2_GuaranteedInvoice_TransactionType_authorization_orderItem, All Shopping basket items (order-items)>> must be specified in every request. + 
The authorized amount must match the total value of the items in the basket.

[%autowidth]
[cols="m,,,,,,,"]
|===
4+|Field (JSON) |Data Type |Required/Optional |Size |Description
...
4+|transaction-type |String |Required |n/a a|The requested transaction type. For _{payment-method-name}_, the transaction-type must be set to ``authorization``.

.2+|requested-amount  
3+m|currency |String |Required |3 a|The currency of the requested/contested
transaction amount. Must be ``EUR``.
3+m|value |Numeric |Required |9.2 a|The full amount that is requested/contested in a transaction. The amount must be between a defined minimum value and a defined maximum value. +
...
.10+|account-holder .5+m|address 2+m|street1 |String |Required |64 |The first line of the consumer's address street.

2+|state |String |Required |32 |The consumer's state.

2+|country |String |Required |2 |The consumer's country code.

2+|city |String |Required |32 |The consumer's city.

2+|postal-code |String |Required |16 |The postal/ZIP code of the consumer's address.

3+m|first-name |String |Required |32 |Consumer's first name.

3+m|last-name |String |Required |32 |Consumer's last name.

3+m|email |String |Required |64 |Consumer's email address.

3+m|phone |String |Required |32 |Consumer's phone number.

3+m|date-of-birth |String |Required |n/a |Consumer's date of birth. {payment-provider-name} validates the consumer's age to check if they are eligible for payment with Guaranteed Invoice.

4+|order-number |String |Required |32 v|Merchant-side order number. +
Allowed characters: [a-zA-Z0-9+]

.6+|[[PPv2_GuaranteedInvoice_TransactionType_authorization_orderItem]]order-items .6+m|order-item .2+m|name m|description |String |Required |100 |Name of the item in the shopping basket.

|article-number |String |Required |40 |EAN or other merchant side article identifier.

.2+m|amount m|value |Numeric |Required |10 |Item’s price per unit.
...
|payment-methods 2+m|payment-method m|name |String |Required |256 |The name of the payment method used for the transaction, i.e. ``ratepay-invoice``.
|===
...
[source,json,subs=attributes+]
----
include::/{base_dir}/samples/json/PPv2GuaranteedInvoiceTransactionTypeAuthorizationRedirectURL.json[]
----
...
[#PPv2_GuaranteedInvoice_AdditionalBusinessContitions]
...
----
include::/{base_dir}/samples/json/PPv2GuaranteedInvoiceTransactionTypeAuthorizationResponse.json[]
----
...
.2+|custom-fields 2+m|field-name |String |The custom field name is ``terms-consent-checked``. The corresponding ``field-value`` indicates whether or not the consumer agreed to the <<PPv2_GuaranteedInvoice_ConsenttoGeneralTermsandConditions, General Terms and Conditions>> by ticking the checkbox.
...
|device 2+m|fingerprint |String |<<GuaranteedInvoice_DeviceFingerprint_GuaranteedInvoice, Click for a detailed fingerprint description>>.
...
IMPORTANT: Check the REST API <<GuaranteedInvoice, {payment-method-name} specification>> for details on {payment-method-name}-specific post-processing operations.
...
[source,json,subs=attributes+]
----
include::/{base_dir}/samples/json/PPv2GuaranteedInvoice_JSONNVP_FieldReference_JSON_StructureForGuaranteedInvoiceRequests.json[]
----
...
[source,json]
----
include::/{base_dir}/samples/json/PPv2GuaranteedInvoice_JSON_NVPFieldsRequest_ResponseOnlyFields.json[]
----
