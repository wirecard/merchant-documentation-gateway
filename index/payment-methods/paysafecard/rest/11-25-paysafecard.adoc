...
|_credit_ |Allows the merchant to send money back to the consumer via paysafecard. |YES |YES |YES |<<paysafecard_Samples_credit, credit sample>>
|===
...
.  Merchant sends a _pending-credit_ request to _{payment-gateway-abbr}_.
.  {payment-gateway-abbr} processes the transaction.
.  {payment-gateway-abbr} sends to the merchant a response with the transaction result.
.  {payment-gateway-abbr} sends to the merchant an IPN with the final transaction status.
...
==== _debit, authorization, capture, void, credit_ transaction
