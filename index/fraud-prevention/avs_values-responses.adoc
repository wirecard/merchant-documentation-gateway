...
The following table outlines the set of codes used by {payment-provider-name}. This uniform code should be the preferred source for decisions related to AVS.
...
The provider response code is the code which the provider returns, together with the human readable provider response message. The following tables list the current set of codes which the card schemes use. It outlines the mapping between the provider response code and the unified {payment-provider-name} response code. 

NOTE: The actual provider response code depends on the provider currently used. Each provider can change or extend the response code list. It may differ from the table below. {payment-provider-name} recommends the preferred usage of the <<FraudPrevention_AVS_{payment-provider-name}ResponseCodes, {payment-provider-name} response code>>.
