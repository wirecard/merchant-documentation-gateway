...
Each card scheme maintains an own set of response codes. To facilitate the
implementation {payment-provider-name} maps those codes to a uniform set of scheme
...
uniform code should be the preferred source for decisions related to AVS.
...
The provider response code is the code returned from the provider. If you use AVS the code is returned in the response, together with the human readable provider response message. The following table lists the current set of codes used by the card schemes and outlines the mapping between the provider response code and the unified {payment-provider-name} response code. Note that the actual provider response code depends on the provider currently used and could also be changed and extended by the provider.
So it may differ from the table below. Therefore {payment-provider-name} recommends the preferred usage of the <<FraudPrevention_AVS_{payment-provider-name}ResponseCodes, {payment-provider-name} response code>>.

// Is "Provider Response Code"= "{payment-provider-name} Response Code" or "card scheme code"?
