...
The provider response code is the code returned from the provider. If you use AVS the code is returned in the response, together with the human readable provider response message. The following table lists the current set of codes used by the card schemes and outlines the mapping between the provider response code and the unified {payment-provider-name} response code. Note that the actual provider response code depends on the actual used provider and could be also changed and extended by the provider.
