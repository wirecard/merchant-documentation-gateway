...
+
IMPORTANT: If a field value contains text with a comma you must enclose the whole field value in double quotation marks (e.g.: value1,“value,2”,value3,”value,4”).
...
.CSV file with comma in value
[source,csv,subs=attributes+]
----
include::{root}/samples/csv/batch-process_comma-value.csv[]
----
<1> The field value for ``order-detail`` is enclosed in double quotation marks because it contains a comma.

Look at our full <<BatchProcessingApi_Sample, Sample>> to see a complete Batch Processing csv file.
