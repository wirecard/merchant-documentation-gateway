...
returned to merchant that can be used for a real authorization or
purchase later.
...
(additional) step. The payment form is displayed directly on merchant’s
checkout page.
...
Merchant will assign call of the function to a pay button on their
checkout page. ``requestData`` are optional when submitting form. They will
...
Merchant can now reference an existing authorization-only transaction to
