...
[IMPORTANT]
====
To test debit transactions, you must include the custom field ``elastic-api.ee.test.result`` in the request.
====
...
``R``: simulate a random payment response (approved or declined) +
``F``: simulate a fraud payment response.
|===
...
[IMPORTANT]
====
To test succesful debit transactions, you must include the custom field ``field_name_1=elastic-api.ee.test.result`` and ``field_value_1=A`` in the request.
====
[%autowidth]
|===
|NVP Fields

|``field_name_1=elastic-api.ee.test.result``
|``field_value_1=A``
