...
|merchant-account-id 2+m|value |String |Required |36 |A unique identifier assigned
...
.2+|requested-amount  2+m|value |Numeric |Required |9.2 a|The full amount that is requested/contested in a transaction. The amount must be between a defined minimum value and a defined maximum value. +
...
.11+|account-holder .5+m|address m|street1 |String |Required |64 |The first line of the consumer's address street.
...
2+m|first-name |String |Required |32 |Consumer's first name.

2+m|last-name |String |Required |32 |Consumer's last name.

2+m|email |String |Required |64 |Consumer's email address.

2+m|phone |String |Required |32 |Consumer's phone number.

2+m|gender |String |Optional |1 a|Consumer' gender: +
``m``=male +
``f``=female

2+m|date-of-birth |String |Required |n/a |Consumer's date of birth. {payment-provider-name} validates the consumer's age to check if they are eligible for payment with Guaranteed Invoice.

3+|order-number |String |Required |32 v|Merchant-side order number. +
Allowed characters: [a-zA-Z0-9+]

.6+|[[PPv2_GuaranteedInvoice_TransactionType_authorization_orderItem]]order-item .2+m|name m|description |String |Required |100 |Name of the item in the shopping basket.
...
2+|payment-method m|name |String |Required |256 |The name of the payment method used for the transaction, i.e. ``ratepay-invoice``.
...
2+|payment-method m|payload |String |The value of ``payload`` depends on the payment method. +
