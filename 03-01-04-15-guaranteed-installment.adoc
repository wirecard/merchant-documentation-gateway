...
Below, we provide samples for the available transaction type <<PPv2_GuaranteedInstallment_TransactionType_authorization, authorization>>, including a field list with short descriptions.
...
- The consumers click *Pay Now* on the payment page. + 
    The payment form window opens.

- The consumers select _{payment-method-name}._ +
    An Installment Calculator is displayed.

- The consumers select one of the available installment plans (number of installments (monthly rates) and the preferred payment day), automatically calculated according to the total sum .
+
WARNING: The minimum amount required for a payment in installments is 60 EUR.
+
- After selecting the installment plan, the consumers need to enter their personal data, shipping and bank information (IBAN and BIC), and send the payment. +
