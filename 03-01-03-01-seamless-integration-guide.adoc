...
``PaymentPage.seamlessRender`` library call to render the payment form in a
...
the ``PaymentPage.seamlessSubmit`` function to submit the payment. Ensure that
...
ifndef::env-wirecard[]
image::images/03-01-03-01-seamless-integration-guide/seamless_ppd_1_whitelabeled.png[Seamless Page Designer 1]
endif::[]
...
Currently, Seamless Mode supports *only credit card payments.* For a more payment-method-specific integration guide, go to <<PPv2_CC, Credit Card with {payment-page-v2-abbr}>>.
...
|CVV              |003
|3D Verification Password |{3ds-pw}
|===
...
----
include::samples/json/WPP_SeamlessIntegrationExampleInitialRequest_RequestHeaders.json[]
----
...
[source,json,subs=attributes+]
----
include::samples/json/WPP_SeamlessIntegrationExampleInitialRequest_CreateAPaymentSessionInitialRequest.json[]
----

|===
 2+|Field (JSON)               |Data Type |Required/Optional |Size |Description

   |_merchant-account-id_ |value |String    |Required          |36   |A unique identifier assigned to every merchant account (by {payment-provider-name}).
...
.2+|_requested-amount_ |value    |Numeric   |Required          |18   |The full amount that is requested/contested in a transaction. 2 decimal places allowed.
   ...
   |_three-d_          |attempt-tree-d |Boolean |Conditional   |N/A  |*Required for 3D Secure transactions.* Indicates whether 3D Secure
...
[source,json,subs=attributes+]
----
include::samples/json/WPP_SeamlessIntegrationExampleRender_ResponseToAuthorizationRequest.json[]
----

Use the *``PaymentPage.seamlessRender``* function to render the payment form in a seamless iframe.
...
<body>
    ...
    <script type="text/javascript">
          ...
          var paymentredirecturl = "https://{test-instance-hostname}/seamless?wPaymentToken=YOUR_TOKEN_HERE";
...
To submit that data, use the *``PaymentPage.seamlessSubmit``* function:
...
<body>
    ...
    <input id="{payment-provider-name}_pay_btn" type="button" onclick="pay()" value="Pay Now"/>
    <script type="text/javascript">
        function pay() {
            PaymentPage.seamlessSubmit({
                onSuccess: function (response) {
...
[source,json,subs=attributes+]
----
include::samples/json/WPP_SeamlessIntegrationExampleParse_4ParseAndProcessThePaymentResponseHighlyRecommended.json[]
----
...
[source,json,subs=attributes+]
----
include::samples/json/WPP_SeamlessIntegrationExampleParse_3DS_DecodedPaymentResponse.json[]
----
...
.2+|_three-d_ e|cardholder-authentication-status |String  |Result of the 3D Secure check.
            e|attempt-three-d                  |Boolean |Indicates whether the transaction should use the 3-D Secure workflow.
|_merchant-account-id_ e|value                   |String  |A unique identifier assigned to every merchant account (by {payment-provider-name}).
.2+|_custom-field_ e|field-name                  |String  |Custom field.
                 e|field-value                 |String  |Custom field.
2+|_transaction-id_                             |String  |A unique identifier assigned to every transaction(by {payment-provider-name}). Used
