...
Typically, a transaction starts with state _in-progress_ and finishes with state either _success_ or _failed_. This information is returned in the response only.
   |_payment-method_ |name                      |String  |The name of the payment method used for the transaction.
.2+|_three-d_ |cardholder-authentication-status |String  |Result of the 3-D Secure check.
            |attempt-three-d                  |Boolean |Indicates whether the transaction should use the 3-D Secure workflow.
|_merchant-account-id_ |value                   |String  |A unique identifier assigned to every merchant account (by Wirecard).
.2+|_custom-field_ |field-name                  |String  |Custom field.
...
.2+|_requested-amount_ |currency                |String  |The currency of the requested/contested transaction amount. +
                                                        Format: 3-character abbreviation according to ISO 4217.
...
.2+|_account-holder_ |first-name                |String  |The first name of the account holder.
                   |last-name                 |String  |The last name of the account holder.

.4+|_card_       |expiration-month              |Numeric |The expiration month of the card used in the transaction.
...
.3+|_status_     |description                   |String  |The description of the transaction status message.
...
.2+|_parent-transaction-amount_  |currency      |String  |The currency of the requested/contested transaction amount. 
                                                        Format: 3-character abbreviation according to ISO 4217.
                               |value         |Numeric |The full amount that is requested/contested in a transaction.

2+|_api-id_                                     |String  |Description of the transaction for account holder’s bank statement purposes.
.2+|_card-token_  |token-id                     |String  |A unique identifier assigned to every card token.
