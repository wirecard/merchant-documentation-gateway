...
h| Secret Key                 | 30520cf9-8f5b-4b0b-9430-58a1223b79dc
|===
...
With _paydirect_ you can send <<PPv2_paydirekt_TransactionType_authorization, _authorization_>> and <<PPv2_paydirekt_TransactionType_debit, _debit_>>.
...
3+| request-id                | String | Required | 36 | A unique identifier assigned to every request (by merchant). Used when
searching for or referencing it later. ``{{$guid}}`` serves as a
placeholder for a random ``request-id``.

Allowed characters:  [a-z0-9-_]
2.2+| requested-amount e| value | Decimal | Required | 7,2 | The full amount that is requested/contested in a transaction. The amount of the decimal place is dependent of the currency. Minimum is 0.01. Maximum is 50000.
...
3+| order-number | String | Required | 20 | The order number provided by the merchant.
3+| transaction-type | String | Required | 22 | The requested transaction type. For _paydirekt_ payments, the
transaction-type must be set to ``authorization``.
.6+| shipping 2+| first-name | String | Required | 32 | The first name of the shipping address.
2+| last-name | String | Required  | 32 | The last name of the shipping address.
.4+| address| street 1| String | Required  | 70 | The first line of the shipping address.
| city | String | Required  | 32 | The city of the shipping address.
| country |String | Required  | 3 | The ``country-id `` of the shipping address.
| postal-code | String | Required | 16 | The postal code or ZIP of the shipping address.
| payment-methods | payment-method      e| name     | String | Required | 9 | The name of the payment method used. Set this value to ``paydirekt``.
...
2.2+| payment-method e| payload | String | Here the merchant can provide detail information about the payment method. It is only used in the response. It can be left empty.
