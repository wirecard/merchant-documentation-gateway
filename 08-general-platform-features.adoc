...
Surcharging is a feature for <<PP, {payment-page-v1}>> (HPP and EPP) and
<<RestApi, REST API>>.
Aimed at merchants who would like to charge additional fees for certain types of
payments, this feature guarantees transparency about surcharge rates for
consumers. In Australia it complies with the
...
Surcharge rates (given as either a fixed amount or a percentage) vary
...
The surcharge amount is the amount added to the original transaction amount (actual price of a good or a service). It is calculated based on respective surcharge rates. Depending on the hight of the originial transaction amount, the surcharge amount varies. The sum of the surcharge amount and the original transaction amount results in the final amount payable.


The surcharge amount can be determined in two ways:

. Auto calculate the surcharge amount based on surcharge rates configured.
. Calculate the surcharge amount yourself and include the value directly as part of the purchase request.
+
NOTE: Including the ``<surcharge-amount>`` as part of the purchase request overwrites your merchant account configuration. It is limited to fixed rate surcharge.
//-
...
Depending on how the surcharge amount is determined, the workflow of a surcharged transaction varies.

If you decide to auto calculate the surcharge amount based on surcharge rates configured 

- send a calculate-fee request and retrieve the surcharge amount from the response first, and 
- insert the retrieved value in and send the purchase request thereafter.
//-

If you decide to cacluclate the surcharge amount yourself, include the value directly as part of the purchase request. 

Below, we provide  <<GeneralPlatformFeatures_Surcharging_Samples_CalculateFee, calculate-fee>> and <<GeneralPlatformFeatures_Surcharging_Samples_Purchase, purchase>> samples.
...
|===
h| URLs (Endpoints) |  ``{rest-api-test-endpoint}``
...
|merchant-account-id  |M  |String |36 |A unique identifier assigned for every merchant account.
|request-id |M |String| 32 a|A unique identifier assigned to every request (by merchant). Used when searching for or referencing it later. _{{$guid}}_ serves as a placeholder for a random _request-id_. Allowed characters: +
a - z +
0 - 9 +
-_
|transaction-type |M  |String |n/a  |The requsted transaction type. Examples are: _authorization, capture, credit, purchase, refund, void authorization, void capture, void credit, void purchase._ For a complete list of transaction types, please see <<AppendixB, Appendix B: Transaction Types>>.
|requested-amount |M |Decimal |18,2 |The full amount that is requested/contested in a transaction. The number of decimals depends on the currency.
|requested-amount@currency |M  |String |3  |This is the currency in which a transaction is received and processed.
|account-holder/last-name |M  |String  |32 |The last name of the account holder.
|account-holder/email |O  |String  |64 |The email address of the account holder.
|card/account-number  |M - Or Card Token  |String |36 |The embossed or encoded number that identifies the card issuer to which a transaction is to be routed and the account to which it is to be charged unless specific instructions indicate otherwise. In the case of a credit card, this is the primary account number.
|card/expiration-month  |M - Or Card Token  |Number|2  |The 2-digit representation of the expiration month of the account-number.
|card/expiration-year |M - Or Card Token  |Number |4  |The 4-digit representation of the expiration year of the account-number.
|card/card-security-code  |Depending on merchant account settings |String |4  |A security feature for credit or debit card transactions, providing increased protection against credit card or debit card fraud. The card security code is located on the back of Mastercard, Visa and Discover credit or debit cards and is typically a se parate group of 3 digits to the right of the signature strip. On American Express cards, the card security code is a printed, not embossed, group of four digits on the front towards the right.
|card/card-type |M |String  |15 |A card scheme accepted by the processing system. This includes physically issued cards.
|surcharge/surcharge-amount | O   | Decimal | 18,6 | This is the amount added to the original transaction amount (actual price of a good or a service) based on respective surcharge rates. The number of decimals depends on the currency.
|payment-methods/payment-method@name  |O  |String  |256  |The name of the payment method.
|descriptor |O |String | n/a |Description of the transaction for account holder's
bank statement purposes.
|merchant-account-id@ref |M  |String  |n/a  |A reference URL to the merchant account. Can be used to retrieve merchant account details over a browser. This information is returned in the response only.
|transaction-id |M |String |36 |A unique identifier to every transaction (by {payment-provider-name}). Used when searching for or referencing to it later. This information is returned in the response only.
|transaction-state  |M  |String |n/a  a|The current transaction state.
...
Typically, a transaction will start from an ``in-progress`` state, and then finish in either the ``success`` or ``failed`` state. This information is returned in the response only.
...
|statuses/status@description |M |String |256 |The description of the transaction status message. This information is returned in the response only.
|statuses/status@severity |M |String |20 a|The definition of the status message. +
Possible values:
...
- ``error`` 
//-
|card-token/token-id  |O |String  |16 |A unique identifier assigned for every card token. This is a surrogate value for the primary account number. This information is returned in the response only.
|card-token/masked-account-number |M  |String |16 |The masked account number. This information is returned in the response only.
|authorization-code |O  |String |24 |A code generated by the card issuing bank as proof that the transaction request was acknowledged or declined. This information is returned in the response only.
|api-id |O  |String |36 |Identifier of the currently used API. This information is returned in the response only.

|original-requested-amount | O  | Decimal | 18,6 | The full amount that is requested/contested in the original transaction to be surcharged. The number of decimals depends on the currency. This information is returned in the response only.
|original-requested-amount@currency |O |String |3 |This is the currency in which the original transaction to be surcharged is received and processed. This information is returend in the response only.
|===
...
[source,xml,subs=attributes+]
----
include::samples/xml/GeneralPlatformFeatures_Surcharging_CalculateFee_request.xml[]
----

.XML Response
[source,xml,subs=attributes+]
----
include::samples/xml/GeneralPlatformFeatures_Surcharging_CalculateFee_response.xml[]
----
...
[source,xml,subs=attributes+]
----
include::samples/xml/GeneralPlatformFeatures_Surcharging_request.xml[]
----

.XML Response
[source,xml,subs=attributes+]
----
include::samples/xml/GeneralPlatformFeatures_Surcharging_response.xml[]
----
