...
- *Issuer-based:* Post-clearing, you receive the full amount from the acquiring bank. The consumer pays their issuing bank (same entity as acquiring bank due to *on-us*) in installments.
...
Several IPPs can be bundled in a **program**. A program is an installment payment framework which defines:
...
The IPPs combined in one program only differ by **tenure**, which is the duration of installment payment. It depends on the financial institutions
and ranges from 3 months to 24 months.

Thus, a specific **IPP is uniquely identified by program and tenure.**

Program examples:

* Zero-interest installments: 3, 6, 12 months
* Pay later in installments with 2% interest: 3, 6, 12 months
...
image::images/08-general-platform-features/IPP_EE_1card.png[width=950, align="center"]
...
. The merchant sends an <<GeneralPlatformFeatures_IPP_InstallmentCalculator, Installment Calculator>> request to WPG to check the card's IPP eligibility.
. The <<GeneralPlatformFeatures_IPP_InstallmentCalculator, Installment Calculator>>:
** validates the credit card number against the BIN ranges activated at the issuing bank. +
...
** checks if the transaction and the transaction currency are domestic.
====
...
[start=5]
. The <<GeneralPlatformFeatures_IPP_InstallmentCalculator, Installment Calculator>> generates a list of all available IPPs and sends it to the merchant.
. The merchant's shop offers "Payment in Installments" to the consumer.
...
. The consumer pays the issuing bank (same entity as acquiring bank since *on-us*) in installments.
...
You receive two additional fields in the response to this request:
...
m|interest-rate
|Decimal
|Interest rate of the selected IPP in percent, e.g. 02.75.
...
This is just a suggestion and may be replaced by any text you find more relevant for your consumer.
