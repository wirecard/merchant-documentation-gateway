...
- Auto-calculate the surcharge amount based on the surcharge rates configured.
...
If you decide to auto-calculate the surcharge amount based on the surcharge rates configured 
...
Below, we provide  <<GeneralPlatformFeatures_Surcharging_Samples_CalculateFee, _calculate-fee_>> and <<GeneralPlatformFeatures_Surcharging_Samples_Purchase, _purchase_>> samples.
...
|request-id |M |String| 32 a| Unique identifier assigned to every request (by the merchant). Used when searching for or referencing it later. _{{$guid}}_ serves as a placeholder for a random request-id. Allowed characters: +
a - z +
0 - 9 +
-_
|transaction-type |M  |String |n/a  | The requested transaction type. + 
For a complete list of transaction types, please see <<AppendixB, Appendix B: Transaction Types>>.
...
|requested-amount@currency |M  |String |3  | The currency in which a transaction is received and processed.
...
|card/account-number  |M - or Card Token  |String |36 | The embossed or encoded number that identifies the card issuer to which a transaction is to be routed and the account to which it is to be charged unless specific instructions indicate otherwise. In case of credit card, this is the primary account number.
...
|card/card-type |M |String  |15 | The card scheme accepted by the processing system. This includes physically issued cards.
|surcharge/surcharge-amount | O   | Decimal | 18,6 | The amount added to the original transaction amount based on the respective surcharge rates. The number of decimals depends on the currency.
