...
ifdef::env-nova[] 
When the merchant wants to receive a notification, he must include the IPN notifications element in each transaction request.

[WARNING]
====
In case a notification address has changed, it is the responsibility of the merchant to request a Firewall Change Request on the merchant's system so that the firewall will accept the new address. 
====

[NOTE]
====
In case there is no format specified in the request then the notification will be sent in ``application/xml``.
The ``application/json`` format must be specified explicitly.
...
.JSON Notifications
The following example shows how you must set the ``<notification>`` element to receive a notification in JSON format:
...
===== Transaction Type in a Notification

The transaction type in the notification may differ from the one sent in the request. This depends on the selected payment method.
E.g. a _pending-debit_ request may result in a _debit-return_ notification or an _authorization_ request may result in a _deposit_ notification. +

NOTE: For credit card transactions ``transaction-id`` indicates a clear reference between request/response and notification. +
For alternative payment method transactions compare the ``transaction-id`` of the response with the ``parent-transaction-id`` of the notification.
...
Referenced transactions (e.g. refund, recurring) have a limited transaction lifecycle in _{payment-gateway-abbr}._ Starting on 1 February 2020 only transactions processed within the last two years can be referenced.
====
endif::[]
...
ifdef::env-nova[]
This reference can be accomplished for all payment methods via the
transaction ID.
endif::[]
...
The following example for _PayPal_ shows the use of
<``parent-transaction-id``>:
