...
. Auto calculate the surcharge amount based on surcharging rates configured.
...
Depending on the option selected, the workflow of a surcharged transaction varies.

If you decide to auto calculate the surcharge amount based on surcharging rates configured (calculate-fee surcharge)
...
If you decide to cacluclate the surcharge amount yourself, include the value directly as part of the purchase request (pass-through surcharge). 

Below, you can find samples for both, the <<GeneralPlatformFeatures_Surcharging_Samples_CalculateFee, calculate-fee request>> and the <<GeneralPlatformFeatures_Surcharging_Samples_Purchase, purchase request>>.
...
|surcharge/surcharge-amount | O   | Decimal | 18,6 | This is the surcharge rate of a transaction. The number of decimals depends on the currency.
