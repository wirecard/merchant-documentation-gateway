...
**I**nstallment **P**ayment **P**lan is a feature that empowers the
consumer to pay larger amounts not at one time but with equal
installments over a certain period of time. +
Depending on the total order amount, the merchant can offer various IPPs to the consumer. These IPPs vary with regard to tenure and installment payment framework (e.g. interest rate). +
 +
IPP parameters have to be contractually agreed upon by merchant and issuing bank. The issuing bank provides Wirecard with these IPP parameters.
...
.IPP Characteristics
- **fixed**: Installments are of equal height.
...
- **credit card:** This feature  is only available for credit card payments. 
+
...
[cols="h,"]
|===
| Countries | HK, ID, MY, PH, SG, TH, TW
| Currencies | HKD, IDR, MYR, PHP, SGD, THB, TWD
|===
//-
...
IPP works with transaction type __purchase__.
...
2. Merchant displays payment form, where consumer is to enter credit card data (PAN).
3. WPG checks card's IPP eligibility by validating the card number against the BIN ranges activated at the issuing bank. 
The BIN range is part of the agreement between merchant and issuing bank. It lists all  **B**ank **I**dentification **N**umbers (1st 6 digits of credit card number) that qualify for installment payment.
//+
...
** Validation OK: Consumer can select either regular payment or payment in installments.
...
contains the order amount and the ISO locale code (including country and language).
5. WPG forwards the request to issuing bank.
6. Issuing bank generates a list of all available IPPs and sends it to merchant.
7. Merchant shows the available installment plans to consumer
(on the checkout page).
8. Consumer selects the appropriate installment plan. By submitting,
   the selected program/tenure is sent with the __purchase__ request to WPG.
   The mandatory IPP fields sent in this __purchase__ request are <<GeneralPlatformFeatures_IPP_Fields, __tenure__>> and <<GeneralPlatformFeatures_IPP_Fields, __program-id__>>. 
9. WPG forwards the request to the issuing bank.
10. Issuing bank processes request and sends response to WPG.
11. WPG forwards response to merchant. 
12. Merchant informs consumer about transaction outcome. 
13. Issuing bank pays the total order amount to merchant.
14. Consumer pays back to their issuing bank (same
  entity as acquiring bank since on-us) in installments.
   
[#GeneralPlatformFeatures_IPP_Fields]
[discrete]
==== Fields
A __purchase__ request with installment payment requires two additional **mandatory** fields that identify the IPP selected by the consumer:
...
Enter the value from the field __months__ from the Installment Calculator response.
...
An additional field is sent in the response to this request:
...
The Installment Calculator is a valuable tool to retrieve all available
installment plans as agreed upon by merchant and issuing bank.
...
|Enumeration
|
|Indicates if the status means __warning__, __error__ or __information__.
...
|Alphanumeric
|36
|This ID uniquely identifies the corresponding IPP. +
 Insert this value as __program-id__ of a subsequent purchase request.
...
|Number of installments (months) to pay for this IPP. +
 Insert this value as __tenure__ of a subsequent purchase request. 
|===
...
<installment-calculator-request xmlns="http://www.elastic-payments.com/schema/utils">
   ...
   <amount currency="SGD">154</amount>
   <locale>en_SG</locale>
