...
Surcharging is an option available in the Wirecard Payment Page, aimed particularly
at regions (Australia) with high fees for the usage of certain card payments.
This option improves the price signals to consumers about the relative costs
of different payment methods.

Surcharging rates (calculated/expressed as either a fixed amount or a percentage
of the regularly paid sum) are differentiated by payment method,
credit card type and indicator, as follows:
...
* JCB
* UPI
- by card type indicator
* credit
* debit
* prepaid
- by alternative payment method (if required)
* POli
* Alipay
* Paypal
* UPOP
...
[#GeneralPlatformFeatures_Surcharging_Setup_Precondition]
====== Precondition

The following preconditions must be met:

- The merchant account must be configured to enable surcharging.
- The country of the merchant must be defined.
- Surcharging rates for merchant accounts must be configured
//-

To enable this feature, a database configuration is required.
The ``<surcharge-amount>`` can be added in the request to overwrite any configuration in database.

NOTE: The ``<surcharge-amount>`` in the request is only limited to fixed
rate surcharge. This feature is introduced into the Australian market for
improved transparency and to eliminate excessive surcharging.

[#GeneralPlatformFeatures_Surcharging_Supported_Flows]
===== Supported Flows

- Wirecard Payment Page V1 - Hosted Payment Page
//-

[#GeneralPlatformFeatures_Surcharging_Test_Credentials]
===== Test Credentials

|===
h| URLs (Endpoints)	| TBD
h| Merchant Account ID (MAID)	|	TBD
...
|transaction-type	|M	|xs:token	|n/a	|Determines a transaction's behavior during transaction processing and merchant settlement. Examples are: authorization, capture, credit, purchase, refund, void authorization, void capture, void credit, void purchase. For a complete list of transaction types, please see <<AppendixB, Appendix B: Transaction Types>>.
