...
Surcharging is a feature for <<PP, Wirecard Payment Page v1>> (HPP and EPP) and <<RestApi, REST API>>. 
Aimed at merchants who would like to charge additional fees for certain types of payments, 
this feature guarantees transparency about surcharging rates for consumers. 
It is currently available in Australia and complies with the 
...
vary depending on region, card type indicator, card type, and payment method.
...
Please <<ContactUs, contact merchant support>> to set up your merchant account as follows:
...
The ``<surcharge-amount>`` can be added in the request to overwrite your merchant account configuration.
NOTE: The ``<surcharge-amount>`` in the request is limited to fixed-rate surcharge.
...
|===
h| URLs (Endpoints)	
 | Please <<ContactUs, contact merchant support>> for full test credentials.
...
|xs:token	
|n/a	
|Determines a transaction's behavior during transaction processing and merchant settlement. Examples are: authorization, capture, credit, purchase, refund, void authorization, void capture, void credit, void purchase. For a complete list of transaction types, please see <<AppendixB, Appendix B: Transaction Types>>.
...
|surcharge/original-requested-amount	
|TBD		
|TBD 
|TBD 
|TBD
