...
Depending on the total order amount, you can can offer a range of payment plans to the consumer. These IPPs differ in tenure and installment payment framework (e.g. interest rate). +
 +
...
Enter the value from the field ``months`` provided in the Installment Calculator response.

m|program-id
|Alphanumeric
|36
|This ID uniquely identifies the corresponding installment payment plan.
