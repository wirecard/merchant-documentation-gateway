...
Several IPPs are combined in a so called **program**. A program is an installment payment framework, defining e.g.
...
Thus, a scecific **IPP is uniquely identified by program and tenure.**
...
. The Merchant offers "Payment in Installments" to the consumer.
