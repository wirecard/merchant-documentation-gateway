...
"periodic" tag is set right in the beginning, in the _Request Initial
iDEAL_.
...
This section describes a set of requests and responses in which the
"periodic" tag is set at a subsequent request, the _SEPA Debit
Recurring_ #1.
...
[[GeneralPlatformFeatures-instant_payment_notificationInstantPaymentNotification]]
[[GeneralPlatformFeatures-instant_payment_notification]]Instant Payment
Notification
------------------------------------------------------------------------------------

[[GeneralPlatformFeatures-Introduction]]
Introduction
~~~~~~~~~~~~
...
[[GeneralPlatformFeatures-IPNConfiguration]]
IPN Configuration
~~~~~~~~~~~~~~~~~
...
can be called up at our mailto:support@wirecard.com[Merchant Support].
...
format in the configuration then NVP response is used for
link:Redirect-URLs-and-IPNs_786603.html[HPP/EPP requests] or
...
[[GeneralPlatformFeatures-RequestaNotificationinJSONFormat]]
...
To request an `application/json-signed `notification, simply switch the
...
*XML Request (determines notification in JSON format)*  Expand source
...
*JSON Notification*  Expand source
...
[[GeneralPlatformFeatures-SignedJSONNotificationExample]]
Signed JSON Notification Example
++++++++++++++++++++++++++++++++

*application/json-signed Notification Example*  Expand source
...
[[GeneralPlatformFeatures-VerifyingtheNotification]]
...
*Verifying the Notification Signature (Java)*  Expand source
...
[[GeneralPlatformFeatures-NotificationwhenrequestedinHTMLFormat]]
Notification when requested in HTML Format
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
image:attachments/1671307/3703458.png[image,height=250]

[[GeneralPlatformFeatures-NotificationwhenrequestedinXMLFormat]]
Notification when requested in XML Format
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
*Notification in XML Format*  Expand source
...
|=======================================================================
|Notification Type |Notification Tag in Request a|
possible extensions
...
|HTTP(S) a|
_`URL, transaction-state`_

 |any configured URL |80, 443, 5500 a|
if _URL_ <> empty and _`transaction-state`_ = empty

all notifications go to that URL

or

if _URL_ <> empty and _`transaction-state`_ <> empty

depending on the _transaction-state_ (success, failed, in-progress, etc)
the notification goes to the corresponding _URL_.

|SMTP a|
`URL, transaction-state`

 |any valid email address |  a|
if _URL_ <> empty and _`transaction-state`_ = empty

all notifications go to that URL (Email address prefixed with ‘mailto’).
...
depending on the _transaction-state_ (success, failed, in-progress, etc)
the notification goes to the corresponding _URL_ (Email address prefixed
with ‘mailto’).

|=======================================================================

A request, which defines a URL to be used, when the payment process will
fail:

*XML Request with different URL for failed transactions*  Expand source
...
[[GeneralPlatformFeatures-NotificationExamples]]
Notification Examples
^^^^^^^^^^^^^^^^^^^^^

[[GeneralPlatformFeatures-CreditCardPayment]]
Credit Card Payment
+++++++++++++++++++

*XML Notification Credit Card*  Expand source
...
[[GeneralPlatformFeatures-AlternativePaymentMethods]]
Alternative Payment Methods
+++++++++++++++++++++++++++

*XML Notification Alternative Payment Method*  Expand source
...
Note that the “amount” value for some alternative payment notifications
contains 6 minor digits.

[[GeneralPlatformFeatures-ContentoftheNotification]]
Content of the Notification
~~~~~~~~~~~~~~~~~~~~~~~~~~~

The content of the notification depends on the status of the response
_Wirecard Payment Gateway_ (WPG) receives from the provider.

[cols=",",options="header",]
|=======================================================================
|WPG receives... |WPG sends notification:...
|a successful response
|_Success_ https://document-center.wirecard.com/display/PTD/Return+Codes+and+Transaction+Statuses[Return
Codes and Transaction Status]

|a failure response |_Failure_
...
Credit Card IPN´s are not encoded and non Credit Card IPN´s are mostly
...
[[GeneralPlatformFeatures-NotificationTransactionTypes]]
...
[cols=",",options="header",]
|=======================================================================
|Request/Response Transaction Type |Possible Notification Transaction
Type
...
[[GeneralPlatformFeatures-NotificationDelay]]
Notification Delay +
~~~~~~~~~~~~~~~~~~~~
...
[[GeneralPlatformFeatures-IPValidation]]
IP Validation
-------------
...
|Private IP: "192.168.0.1" |Regular IP:
"32b1:953e:207a:1d36:24bf:9244:20a0:7cea"
...
[[GeneralPlatformFeatures-RedirectURL]]
Redirect URL
------------
...
The merchant can determine up to
https://document-center.wirecard.com/display/PTD/General+Platform+Features#GeneralPlatformFeatures-RedirectURLsDeterminedinRequest[three
different __redirect URL__s in one request]. In that case the merchant
can determine one success URL, one for failure and one cancellation
URL. +

[[GeneralPlatformFeatures-RedirectURLsconfiguredinMAID]]
Redirect URLs configured in MAID
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

In this request the redirect URLs are configured on MAID level.

*General Request Sample for Redirect URL*  Expand source
...
[[GeneralPlatformFeatures-RedirectURLsDeterminedinRequest]]
...
<payment xmlns="http://www.elastic-payments.com/schema/payment">
    ...
    <fail-redirect-url>https://www.merchant-website.com/fail.html</fail-redirect-url>

    <cancel-redirect-url>https://www.merchant-website.com/cancel.html</cancel-redirect-url>

    <success-redirect-url>https://www.merchant-website.com/success.html</success-redirect-url>
...
[[GeneralPlatformFeatures-InstrumentCountryBounceback]]
...
[[GeneralPlatformFeatures-MerchantAccountResolving]]
Merchant Account Resolving
--------------------------

[[GeneralPlatformFeatures-Introduction.1]]
Introduction
~~~~~~~~~~~~
...
link:#GeneralPlatformFeatures-resolving_rules[resolving rules].
...
[[GeneralPlatformFeatures-resolving_rulesResolvingRules]]
[[GeneralPlatformFeatures-resolving_rules]]Resolving Rules
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
[[GeneralPlatformFeatures-ResolverCategoryCode]]
Resolver Category Code
~~~~~~~~~~~~~~~~~~~~~~

_Resolver Category Code_ a is field that defines groups of several
...
[[GeneralPlatformFeatures-SupportedPaymentMethods]]
...
[[GeneralPlatformFeatures-WhatResolverscanbespecified]]
What Resolvers can be specified
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
[[GeneralPlatformFeatures-SuperMerchant]]
Super Merchant
~~~~~~~~~~~~~~
...
The payment *response* (structured payment by seamless or message POST
to merchant's page) *contains* resolved merchant and *signature computed
with secret key of resolved merchant*.

[[GeneralPlatformFeatures-Transactions]]
Transactions
------------

[[GeneralPlatformFeatures-ReferencingaTransaction]]
...
https://document-center.wirecard.com/display/PTD/Payment+Features#PaymentFeatures-Tokenization[use
a token].
...
Be aware that the _parent transaction ID_ is also referred to as
_reference transaction ID_.
...
|=======================================================================
|*Transaction No.* |*Transaction Type* |*<transaction-id>*
|*<parent-transaction-id>*
...
[[GeneralPlatformFeatures-RecurringTransaction.1]]
Recurring Transaction
~~~~~~~~~~~~~~~~~~~~~
...
insurance). A
https://document-center.wirecard.com/display/PTD/General+Platform+Features#GeneralPlatformFeatures-ReferencingaTransaction[reference
to such a transaction] always means that the subsequent transaction has
...
supports recurring transactions with the payment methods
https://document-center.wirecard.com/display/PTD/Credit+Card#CreditCard-RecurringTransaction[Credit
Card],
https://document-center.wirecard.com/display/PTD/PayPal#PayPal-RecurringTransaction[PayPal]
and
https://document-center.wirecard.com/display/PTD/SEPA+Direct+Debit+Recurring+Transaction[SEPA].
...
[[GeneralPlatformFeatures-ThePeriodicType]]
The Periodic Type__ +
__
+++++++++++++++++++++
...
[[GeneralPlatformFeatures-TheSequenceType]]
The Sequence Type
+++++++++++++++++
...
[[GeneralPlatformFeatures-Restrictions]]
Restrictions
++++++++++++
...
[[GeneralPlatformFeatures-RetrieveaTransaction]]
...
In the standard process, the merchant is informed via
https://document-center.wirecard.com/display/PTD/General+Platform+Features#GeneralPlatformFeatures-instant_payment_notificationInstantPaymentNotification[IPN]
...
There are
https://document-center.wirecard.com/display/PTD/General+Platform+Features#GeneralPlatformFeatures-NotificationDelay[several
reasons] why _Retrieve Transaction_ shall not be launched immediately
after submitting a transaction and still wating for the IPN. +

[[GeneralPlatformFeatures-FormatsandMethods]]
Formats and Methods
^^^^^^^^^^^^^^^^^^^
...
[[GeneralPlatformFeatures-RetrieveTransactionbyTransaction-ID]]
...
The
https://document-center.wirecard.com/display/PTD/General+Platform+Features#GeneralPlatformFeatures-instant_payment_notification[Notification
file (IPN)], the merchant gets (based on configuration), is the standard
...
[[GeneralPlatformFeatures-Endpoint]]
Endpoint
++++++++
...
[[GeneralPlatformFeatures-Fields]]
Fields
++++++
...
[[GeneralPlatformFeatures-SampleURLtoaMerchant'sEndpoint]]
...
[[GeneralPlatformFeatures-response_retrieve_transactionResponsewhenretrievingatransaction]]
[[GeneralPlatformFeatures-response_retrieve_transaction]]Response when
...
[[GeneralPlatformFeatures-Retrieve_by_Request-IDRetrieveTransactionbyRequest-ID]]
[[GeneralPlatformFeatures-Retrieve_by_Request-ID]]Retrieve Transaction
by Request-ID
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
[[GeneralPlatformFeatures-Endpoint.1]]
Endpoint
++++++++
...
[[GeneralPlatformFeatures-Fields.1]]
Fields
++++++
...
[[GeneralPlatformFeatures-SampleURLtoaMerchant'sEndpoint.1]]
Sample URL to a Merchant's Endpoint
+++++++++++++++++++++++++++++++++++
...
[[GeneralPlatformFeatures-Responsewhenretrievingatransaction]]
...
https://document-center.wirecard.com/display/PTD/General+Platform+Features#GeneralPlatformFeatures-response_retrieve_transactionResponsewhenretrievingatransaction[Retrieve
Transaction by Transaction-ID].

[[GeneralPlatformFeatures-Retrieve_by_Request-ID_CategoryRetrieveTransactionbyRequest-IDandCategory]]
[[GeneralPlatformFeatures-Retrieve_by_Request-ID_Category]]Retrieve
Transaction by Request-ID and Category
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
[[GeneralPlatformFeatures-Endpoint.2]]
Endpoint
++++++++
...
[[GeneralPlatformFeatures-Fields.2]]
Fields
++++++
...
[[GeneralPlatformFeatures-SampleURLtoaMerchant'sEndpoint.2]]
Sample URL to a Merchant's Endpoint
+++++++++++++++++++++++++++++++++++
...
[[GeneralPlatformFeatures-Sample:XMLAuthorizationResponse]]
Sample: XML Authorization Response
++++++++++++++++++++++++++++++++++

*XML Authorization Response (Successful)*  Expand source
...
[[GeneralPlatformFeatures-StoringAdditionalDatawithCustomFields]]
Storing Additional Data with Custom Fields
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

In addition to processing transactions, Wirecard’s Payment Gateway also
...
In the following example, the client application has stored a _purchase
order_, invoice, _crm-id_, _customer-tier_ and _promotional-code_.
...
[[GeneralPlatformFeatures-ViewTransactionsintheBrowser]]
...
* \{hostname}: +
** if test environment:
http://api-test.wirecard.com[api-test.wirecard.com]
** if production environment: http://api.wirecard.com[api.wirecard.com]
...
[[GeneralPlatformFeatures-TransactionTypes]]
Transaction Types
~~~~~~~~~~~~~~~~~

[[GeneralPlatformFeatures-check-signature/authorisation]]
_check-signature/authorisation_
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

[[GeneralPlatformFeatures-Description]]
Description
+++++++++++

_check-signature_ and its related transaction types are transaction
types that are dependend on the payment method not of elematary need for
processing via _Wirecard Payment Gateway_. Some payment methods just
provide the possibity to verfify if this payment method is allowed to
...
For
https://document-center.wirecard.com/pages/viewpage.action?pageId=786562[Sofort.]
the transaction type _check-signature_ can be submitted to validate the
digital signature.

[[GeneralPlatformFeatures-get-url/pending-debit/pending-credit]]
_get-url/pending-debit/pending-credit_
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

[[GeneralPlatformFeatures-Description.1]]
Description
+++++++++++

_get-url_ and its related transaction types are transaction types that
depend on the payment method of elemenatary need for processing via
_Wirecard Payment Gateway_. Because of that for some payment methods
they are additionaly created automatically even they were not sent via
the integration option. In WEP these Transaction Types are visible.

Example

For https://document-center.wirecard.com/display/PTD/SEPA+Direct+Debit[SEPA
Direct Debit] the transaction type _pending-credit_ is created in
...
[[GeneralPlatformFeatures-auto-sale]]
_auto-sale_
^^^^^^^^^^^

[[GeneralPlatformFeatures-Description.2]]
Description
+++++++++++
...
[[GeneralPlatformFeatures-BasicConfiguration]]
Basic Configuration
+++++++++++++++++++
...
[[GeneralPlatformFeatures-ExtendedConfiguration]]
...
[[GeneralPlatformFeatures-CurrencyConfiguration]]
...
[[GeneralPlatformFeatures-Example]]
Example
...
[[GeneralPlatformFeatures-ServerAvailability]]
Server Availability
-------------------
...
https://api.wirecard.com/engine/status/[https://api.wireard.com/engine/status/]
...
link:attachments/1671307/3114224.jpg[process_merchant_wd_bank.jpg]
(image/jpeg) +
image:images/icons/bullet_blue.gif[image,width=8,height=8]
link:attachments/1671307/3114225.png[sample recon file table.png]
(image/png) +
image:images/icons/bullet_blue.gif[image,width=8,height=8]
link:attachments/1671307/3114226.xlsx[20170711_WPGRecon_All refactored
final.xlsx]
(application/vnd.openxmlformats-officedocument.spreadsheetml.sheet) +
image:images/icons/bullet_blue.gif[image,width=8,height=8]
link:attachments/1671307/3114229.jpg[file download_file directory.jpg]
(image/jpeg) +
image:images/icons/bullet_blue.gif[image,width=8,height=8]
link:attachments/1671307/3114230.jpg[file download_retrieving files.jpg]
(image/jpeg) +
image:images/icons/bullet_blue.gif[image,width=8,height=8]
link:attachments/1671307/3114259.png[add products_tokenize.png]
(image/png) +
image:images/icons/bullet_blue.gif[image,width=8,height=8]
link:attachments/1671307/3114324.csv[APTransactions_195.175.175.175_2017-08-23_merchantID.csv]
(application/vnd.ms-excel) +
image:images/icons/bullet_blue.gif[image,width=8,height=8]
link:attachments/1671307/3114322.csv[APTransactions_195.175.175.175_2017-08-23_merchantID.csv]
(text/csv) +
image:images/icons/bullet_blue.gif[image,width=8,height=8]
link:attachments/1671307/3703458.png[HTML Notification.png]
(image/png) +
