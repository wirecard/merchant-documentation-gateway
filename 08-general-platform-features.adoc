...
** minimum amont for installment payments: Only payments with a total amount
...
- Merchant account must be set to "installment plan enabled".
- Merchant account must be set to "on-us card routing enabled".
...
3. WPG checks card's IPP eligibility by validates the card number against the
<<GeneralPlatformFeatures_IPP_Setup, BIN ranges>> activated at the issuing bank.
4. Merchant displays available IPPS for consumer.
5. Consumer selects IPP. By submitting, the data are sent to WPG.
6. WPG forwards the data to the Installment Calculator.
7. Installment Calculator calculates installment amount and sends the
   to merchant.
