...
|original-requested-amount | O  | Decimal | 18,6 | The full amount that is requested/contested in the original transaction. The number of decimals depends on the currency. This information is returned in the response only.
|original-requested-amount@currency |O |String |3 |This is the currency in which the original transaction is received and processed. This information is returend in the response only.
|===
