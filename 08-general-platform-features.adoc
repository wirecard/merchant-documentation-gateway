...
+
NOTE: Configuring surcharging rates allows to auto calculate and query the surcharge amount. If you prefer to calculate the surcharge amount yourself, add the calculated  ``<surcharge-amount>`` in the payment request directly. It overwrites your merchant account configruation. Adding the calculated ``<surcharge-amount>`` in the payment request is limited to fixed rate surcharge.
//-
...
The worklflow of a surcharged transaction varies whether you decide to auto calculate and query the surcharge amount based on configured surcharging rates or to calculate the surcharge amount yourself and add it to the purchase request directly.

If you decide to auto calculate and query the surcharge amount based on configured surcharging rates, a two step procedure is required:

**Step 1:**

. The merchant sends an calculate-fee request to WPG.
. WPG retrieves the surcharge information form  the merchant account setup, calucaltes the surcharge amount and sends the response to WPG.

**Step 2:**
. The merchants sends a purchase request to WPG
