...
default invoice due date is 14 days after capturing.
...
.Invoicing
Depending on your merchant configuration, there are two options of invoicing triggered by a _capture-authorization_:

- _Klarna_ sends an *invoice* to your consumer.
- _Klarna_ sends *payment instructions* to your consumer. You are responsible to send the *invoice* to your consumer. The payment instructions indicate the _Klarna_ bank account details.
//-

According to the provided delivery option, _Klarna_ sends the invoice/payment instructions per:

  ** Email
  ** Mail
  ** Download link enclosed in the delivered package. +
The download link is provided regardless of the invoice delivery
option within the response. It is valid for 30 days. 
//-
...
NOTE: {{adjustTrxId}} serves as placeholder. Enter the ``transaction-id`` of the preceding _authorization_ or _adjust-authorization_ response.
