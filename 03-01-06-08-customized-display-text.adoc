...
If you want to use _Customized Display Text_, add ``customized-display-text`` in the initial POST request and insert a short text (see <<PPv2_CustomizedDisplayText_ExampleRequest, example request>> below). 

The inserted text then appears on the payment page (for HPP) or in the iframe (EPP). 

By default, payment page and iframe displays the first four lines of your text. 

If the defined text requires more than four lines, consumers can expand the field using the arrow at the bottom of the field.
