...
FPX->>{payment-gateway-abbr}: sends transaction status to {payment-gateway-abbr}
...
. {payment-gateway-abbr} initiates the transaction and sends consumer to _FPX_ page showing the details of the bank the consumer has selected.
. _FPX_ sends consumer to desired bank page.
. Consumer logs in and completes payment on bank page.
. {payment-gateway-abbr} receives transaction status from _FPX_.
. {payment-gateway-abbr} sends notification with transaction status to merchant.
. {payment-gateway-abbr} redirects consumer back to the shop where the status of the payment process is displayed by the merchant.
