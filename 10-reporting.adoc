...
. <<Reporting_DataReconciliation, Data Reconciliation>>
. <<Reporting_IcFeeReport, IC Fee Report>>
...
payment methods within _Wirecard´s Payment Gateway_: <<PaymentMethods, Payment Methods>>
...
possible to download files manually it is recommended to automate the
...
``<identifier><ip><date><merchant account id>.csv``
...
shows the exact date, the file has been generated.
- ``<Merchant Account ID>``, is a 36 character identifier specifying your
business uniquely in the Wirecard system.
- ``.csv`` is the file extension denoting the format _Comma Separated Value_.
...
The file ``EngineAPTransactions*.csv`` currently comprises transfer transaction requests for
<<PaymentMethods, the payment methods>> that have been
...
| PAYMENT METHOD                  | This is the name of the payment method that was processed via the Wirecard Payment Gateway.
| TRANSACTION ID                  | This field contains the Transaction ID that was created by the Wirecard Payment Gateway for the initial XML request message.
...
Reconciliation, displayed with a spreadsheet application:

.Example AP Data Reconciliation File for Wirecard Payment Gateway
...
this transaction will be created as unmatched transaction.

It will appear in the reconciliation file without a reference
transaction ID.

If this unmatched transaction is manually matched to an existing
payment on the same day as it has entered the Wirecard system, it will
appear as a transaction within the payment to which it has been matched.

NOTE: There will be no record of an unmatched payment in the reconciliation
file.
...
Although manual downloads are possible it is recommended to use an
automated process.
...
Therefore it is recommended that the merchant always save downloaded
...
If you don’t know your personalized access information,
...
the root window of the remote SFTP server site (Wirecard server) you
will see the following subdirectories:
...
. Copy the desired CSV files to your local machine. It is recommended
...
. The _Wirecard Payment Gateway_ allows you to archive reconciled
...
Since December 2015 the IRF (EU regulation) has required to provide
...
cancellations, with their transactions, aggregated per each transaction
...
An _ICF-Overview Report_ will only be generated, if there had been
...
``Interchange__CustomerID__MerchantShortName_-MerchantAccountShortName__StartDate_EndDate_ReportPeriod.csv``
(_ReportPeriod_ is "monthly").

The detailed report file's name format is: ``InterchangeDetailed__CustomerID__MerchantShortName-_MerchantAccountShortName__StartDate_EndDate_ReportPeriod_v1.csv``
...
[cols="21,17,15,13,6,45,3"]
|===
| Field Name            | Cardinality (EPA) | Data Type (EPA) | Format        | Size   | Description | Appearance

| Invoice Number        | opt.              | String          | Text          | 36     | Unique identifier of invoice or cancellation. | M/D
| Invoice State         | man.              | String          | Text          | 10     | Values: ``Signed`` or ``Cancelled``. | M/D
| Invoice Type          | man.              | String          | Text          | 15     | Values: ``Invoice`` or ``Cancellation``. | M/D
| Reference Invoice ID  | opt.              | String          | Text          | 36     | Id of invoice to which the Cancellation was signed for. | M/D
| Billing Period        | opt.              | yyyy-mm-dd      | Date Interval | 25     | Period we're picking the transactions for. | M/D
...
| Transaction Count     | man.              | Number          | Number        | Number | Count of Transaction of same type within same invoice. | M
...
regular basis *on the 8th day of each month at 8 pm*. The reports are
