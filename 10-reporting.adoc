...
and are saved in CSV format on the SFTP server. When opened and
...
| TRANSACTION ID                  | This field contains the Transaction ID that was created by _{payment-gateway}_ for the initial XML request message.
...
| PTRID                           | Provider Transaction Reference ID: This ID represents the end-to-end flow of a transaction and is used to identify transactions within the banking system. It is generated automatically.
...
To be able to connect to the SFTP server you must have an SFTP
...
the SFTP server to external storage and will no longer be
