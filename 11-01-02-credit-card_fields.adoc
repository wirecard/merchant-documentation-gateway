...
Parent elements (also refered to as _Complex type_) are organised in subordinate child tables (such as ``address``). The fields themselves are part of the respective parent element table (e.g. <<CC_Fields_xmlelements_request_address, address>> is a child table of <<CC_Fields_xmlelements_request_accountholder, account-holder>>. The fields of ``address`` are described in a child table of ``account-holder``). The parent element table provides a link to the table of the child element.
