...
In the section _Transaction Types_ you may, in some occasions, also find a field
list for each transaction type. This field list lists only those fields whose
definition deviates from the definition given here. When you look at the field
list at the individual transaction type, it is assumed that you have knowledge
of this general field list described here.
...
Look, for example, at the ``periodic``-object. The ``periodic``-fields are not
necessary and therefore optional, when you send a request without a recurring
intention but they are mandatory, when you send a set of recurring transactions.
...
<<CC_Fields_xmlobjects_request_avs, avs>>
...
``account-holder`` is a subcategory of ``payment`` and is optional in most of the
cases. Only ``last-name`` is mandatory. Please provide all the ``account-holder``
data in your request to make fraud tests easier.
...
``address`` is a subcategory of ``account-holder``, ``airline-industry`` and
``shipping``. It is used to specify the consumer's address. Data can be provided
optionally but it helps with fraud checks, if the ``address`` is complete.
...
[#CC_Fields_xmlobjects_request_avs]
.avs
[cols="30,6,9,7,48a"]
|===
|Field |Cardinality |Datatype |Size |Description
...
``card`` is a subcategory of ``payment`` and is only sent in the first
transaction request when the card is used for the first time. Due to
<<CreditCard_PaymentFeatures_Tokenization_Introduction, PCI DSS>>
compliance these data is transferred to a token immediately. Beginning with the
...
with this credit card. Token data is provided in the fields ``card-token``.
...
``card-token`` is a subcategory of ``payment`` and is the substitute for ``card``.
Due to <<CreditCard_PaymentFeatures_Tokenization_Introduction, PCI DSS>>
...
[#CC_Fields_xmlobjects_request_statuses]
.statuses
[cols="30,6,9,7,48a"]
|===
|Field |Cardinality |Datatype |Size |Description
