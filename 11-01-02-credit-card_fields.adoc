...
//// 
card-emv, card-pin, card-raw must be removed from table!
////
...
<<CC_Fields_xmlelements_request_card, ``card``>> object.
...
4+| card-types 
| This is the parent of multiple <<CC_Fields_xmlelements_request_cardtype, ``card-type``>> elements. 

4+| payload 
| This is the parent of multiple <<CC_Fields_xmlelements_request_payload, ``payload-field``>> elements. 
|===
...
<<CC_Fields_xmlelements_request_payment, ``payment``>> object.
...
``avs`` belongs to the <<CC_Fields_xmlelements_response_payment, ``payment``>>
