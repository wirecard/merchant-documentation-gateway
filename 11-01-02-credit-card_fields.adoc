...
NOTE: Some transaction types may require a different field definition. If this is the case, the different definition is given directly with the transaction type.
...
// What kind of hierarchy does here apply?
...
Each element is displayed in an own table. The element tables are sorted alphabetically.


[#CC_Fields_intro_elements]
====== Objects

Each field table has the following columns:
...
- <<CC_Fields_intro_elements_Size, Size>>
...
The structure shows a hierarchical relation between the elements. We have a parent element consisting of children. Some children can turn into a parent element themselves and have their own children.

According to the above example we have the parent element ``order-items`` and its child ``order-item``. +
``order-item`` itself is also a parent element of its child ``article-number``. 
The hierarchy levels are sparated by a dot ``.``.

[#CC_Fields_intro_elements_cardinality]
._Cardinality_

_Cardinality_ indicates the functional necessity of the field for the request. A field can be either mandatory (M) or optional (O).
...
NOTE: If the cardinality depends on the context it will be marked with "M/O". The _Description_ will explain the conditions in which the field is mandatory.
...
In the tables below we describe the cardinality considering the technical requirement. We provide suggestions in the description of the tables in which context it makes sense to send a field even though its technical cardinality is _Optional_.
...
Fields containing child fields (such as ``order-item.article-number``) provide a link, which points to the table describing the children.

- _String_ + 
allows a combination of text and numbers which is arbitrary but finite in size. A subset of _String_ values can be defined according to a pattern value, such as ``api-id``. The pattern will be given in the _Description_ column.
- _Boolean_ + 
has one of two possible values (usually either _True_ or _False_).
- _Date_ + 
is a value taken from a calendar (e.g. the date of birth). {date-pattern} This pattern cannot be customized.
- _Timestamp_ + 
Is a moment in time defined by a system. {timestamp-pattern} This pattern cannot be customized.
- _Decimal_ + 
is a numeric value which has digits before the comma and after the comma, such as the amount of money in a specific currency (e.g. 138,53 €).
- _Number_ + 
is a numeric value that has no digits after the decimal, such as expiration month of a credit card.
- _Enumeration_ + 
is a list of predefined values, e.g. the severity of a status, which can provide the predefined values _warning_, _error_ and _information_.

[#CC_Fields_intro_elements_Size]
._Size_
_Size_ shows how many characters can be used for this field. The value given here is the maximum value merchants can use to make sure, the whole amount of characters will be processed.
...
[#CC_Fields_intro_ParentElement]
====== Parent Element and its Children
Parent elements (also refered to as _Complex type_) are organised in subordinate child tables (such as ``address``). The fields themselves are part of the respective parent element table (e.g. <<CC_Fields_xmlelements_request_address, address>> is a child table of <<CC_Fields_xmlelements_request_accountholder, account-holder>>. The fields of ``account-holder`` are described in a child table of ``payment``). The parent element table provides a link to the table of the child element.
...
//vhauss: What do we do? Install a new "periodic" table? > see line 309 and lines 907 to 930!
...
The fields can be ignored if no recurring payment is inteded. For all recurring payments the ``periodic`` fields are mandatory.

[cols="30,6,9,7,48a"]
|===
| Field | Cardinality | Datatype | Size | Description

| periodic-type | M/O | String | 9 | Indicates the recurring payment type. This can be +
``recurring``: is used for the recurrung payments of a subscription.
``installment``: is used for the partial payments of an installment payment.
``ucof``: is used for the .
``ci``: is used for the .
| sequence-type | M/O | String | 9 | Indicates the phase of a recurring transaction. This can be +
``first``: is used for the .
``recurring``: is used for the .
``final``: is used for the .
| recurring-expire-date| M/O| Date | 10| Date after which no further recurring payments using this card are allowed. {date-pattern}
| recurring-frequency| M/O | Number| 4| The minimum number of days between individual payments.   
|===
////
