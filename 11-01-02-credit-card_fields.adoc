...
:timestamp-pattern: The timestamp format is YYYY-MM-DDThh:mm:ss.
...
This is the general field table for _Credit Card_ transactions.

NOTE: Some transaction types may require different field definitions. If this is the case, the different definition is given directly with the transaction type.
//KKS: Field definitions may vary by transaction type. Deviations from standard field definitions are given with the respective transaction type.
...
Within _Request_ and _Response_, the fields are sorted by parent and child elements (such as _payment_, _account-holder_, _card_, _shipping_, etc).
Each element is displayed in a separate table. The element tables are sorted alphabetically.
...
//KKS: please create a mermaid graphic for this; we need to make sure we have consistency in our new graphics
