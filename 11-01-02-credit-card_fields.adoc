...
:timestamp-pattern: Format: ``YYYY-MM-DDThh:mm:ss``.
...
|_Date_ 
|Value taken from a calendar (e.g. the date of birth). {date-pattern} This pattern cannot be customized.

|_Timestamp_
|A moment in time defined by a system. {timestamp-pattern} This pattern cannot be customized.
...
|_Number_
|A numeric value that has no decimal places, such as expiration month of a credit card.
...
If other rules apply for a field, they will be stated in the _Description_ column of that field.

[#CC_Fields_intro_ParentElement]
===== Complex types
////
Parent elements (also referred to as _Complex type_ fields) contain subordinate child tables (such as ``account-holder.address``). The fields themselves are part of the respective parent element table (e.g. <<CC_Fields_xmlelements_request_address, address>> is a child table of <<CC_Fields_xmlelements_request_accountholder, account-holder>>. The ``address`` fields are listed in a child table of ``account-holder``). The parent element table provides a link to the table of the child element.
////

The tables are ordered hierarchically. ``payment`` is the top parent element and all fields that are part of ``payment`` are listed in the first table <<CC_Fields_xmlelements_request_payment, provided here>>. 
Some fields listed in this table are _complex type_ fields, e.g. ``payment.account-holder``. _Complex type_ fields are parents to a number of further fields. For example, ``payment.account-holder`` is a parent of ``address``. Click on the link provided in the table to view the fields contained in a _complex type_ field.

[#CC_Fields_xmlelements]
===== XML Elements
Click the X in the respective column for request and response fields.

Please refer to our <<Appendix_Xml, XSD>> for field descriptions.
