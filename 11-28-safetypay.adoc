...
Consumer’s bank account. | YES | YES | YES |https://document-center.wirecard.com/display/PTD/SafetyPay#SafetyPay-debit[_debit_
example]
| _refund-debit_ |Initiates the _refund_ process, referring to an
existing _debit_ transaction. | YES | YES | YES |https://document-center.wirecard.com/display/PTD/SafetyPay#SafetyPay-refund[_refund_
example]
| _refund-request_ |Returns the original transaction amount to the
Consumer's account. It follows _refund-debit_ as the final step of the
_refund_ flow. | NO | NO | YES | 
| _get-url_ |Retrieves the URL of the _SafetyPay_ landing page for the
Consumer to be redirected to. | NO | YES | YES | 
|===

\* YES indicates that the transaction type is sent in the request.

\\** YES indicates that the transaction type appears in either response or
notification.

\*** YES indicates that the transaction type is visible in Wirecard
Enterprise Portal.
...
image::images/11-27-safetypay/SafetyPay_workflow_debit_restapi.png[SafetyPay workflow for debit using restapi]
...
image::images/11-27-safetypay/SafetyPay_workflow_debit_hpp.png[SafetyPay workflow for debit using HPP]
...
_refund_ is only possible via ReST API.

image::images/11-27-safetypay/SafetyPay_workflow_refund.png[SafetyPay workflow for refund]
...
The fields used for __SafetyPay __requests, responses and notifications
...
[#SafetyPay_Samples:XMLRequestsResponsesandNotifications]
