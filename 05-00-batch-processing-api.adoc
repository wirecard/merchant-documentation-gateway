...
table <<BatchProcessingApi_SepaRequest>>.

//-

Please refer
to <<BatchProcessingApi_Sample>> for Batch Processing.
...
The <<BatchProcessingApi_Fields_CreditCard>> describes the fields that must be provided for Credit Card
...
| sequence_type                  | E          | Empty        | 10   | “Provide empty value” for this field by using the construct ``,,``. For a sample, please look at the <<BatchProcessingApi_Sample>>.
|===
