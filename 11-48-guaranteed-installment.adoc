...
[%autowidth,stripes=none]
|===
h| URLs (Endpoints)           | `\https://{test-instance-hostname}/engine/rest/paymentmethods`
...
[#GuaranteedInstallment_StandardProcess]
===== Standard Process

[#GuaranteedInstallment_GuaranteedInstallment]
====== Guaranteed Installments
...
Merchant->>{payment-provider-name} Bank: collects the transaction amount from consumer's bank account
----
...
. Merchant sends a _capture_ request to {payment-gateway-abbr} when the items
...
[#GuaranteedInstallment_Features_Bankaccountdetails]
...
[#GuaranteedInstallment_DeviceFingerprint_GuaranteedInstallment]
===== Device Fingerprint (Guaranteed Installments)

[#GuaranteedInstallment_DeviceFingerprint_GuaranteedInstallment_Introduction]
====== Introduction

_Device Fingerprint (Guaranteed Installments)_ is a method of fraud
...
[#GuaranteedInstallment_DeviceFingerprint_GuaranteedInstallment_UniqueToken]
...
[#GuaranteedInstallment_DeviceFingerprint_GuaranteedInstallment_RunJavaScript]
====== Run JavaScript
...
[#GuaranteedInstallment_DeviceFingerprint_GuaranteedInstallment_DataTransferToWirecard]
====== Data Transfer to Wirecard

Please transfer the ``deviceIdentToken`` within the authorization request to
Wirecard. You can find a corresponding XML sample for either <<GuaranteedInstallment_Samples_GuaranteedInstallment, Guaranteed Installments>> or <<GuaranteedDirectDebit_Samples_DirectDebit, Guaranteed Direct Debit>>.

// if "Data Transfer to Wirecard" is part of "Installment", check links!!

[#GuaranteedInstallment_RegularConsumers]
===== Regular Consumers
...
[#GuaranteedInstallment_ConsenttoGeneralTermsandConditions]
...
[#GuaranteedInstallment_AdditionalBusinessContitions]
====== Additional Business Conditions

When consumers use _Guaranteed Installments,_
