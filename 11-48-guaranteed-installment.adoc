...
|months|M/O|Integer|Size ?????|Number of months to cover the duration of the installment plan. It is *mandatory* for calculation by time. It must not be used when calculating the installment plan by rate.
...
|payment-method|String|15|The payment method to calculate the installment for. Must be _ratepay-install_.
|merchant-account-id|String|36|Merchant account ID. Must follow a pattern like: 815c32ab-486d-4e4c-baf8-f412e2e4e9b8.
|amount|Decimal |Size ?????|Full amount of the purchase. Must be in the format of _Money_ (Example: 100.00)
|currency (attribute of ``amount``)|String|3|The currency of the purchase. _Guaranteed Installments_ requires EUR.
|rate|Decimal|Size ?????|Monthly rate which the consumer wants to pay. Must be in the format of _Money_ (Example: 25.00).
|currency (attribute of ``rate``)|String|3|The currency of  the rate. _Guaranteed Installments_ requires EUR.
|state|String|Size ?????| The state of the response. Values can be +
``success`` +
``failed``.
...
This field table describes the structure of the response statuses.
[%autowidth,stripes=none, cols="v,,,"]
|===
| Field | Datatype | Size | Description 

|code (attribute of ``status``)|String|Size ?????|The status code of the response (Example: 200.0000)
|description (attribute of ``status``)|String|Size ????? |The description to the status code of the response (Example: "Successful Response"")
|severity (attribute of ``status``)|String|Size ?????|This field informs about the status of the transaction. Values can be +
``information`` +
``error``.
...
This field table describes the structure of the available installment plans which are sent in the response. It is a breakdown of fees and other information about the corresponding installment plan.
[%autowidth,stripes=none, cols="v,,,"]
|===
| Field | Datatype | Size | Description 

|interest-rate|Decimal|Size ?????|Interest-rate which will be used to calculate the annual percentage rate (Example: 12.9).
|monthly-debit-interest|Decimal|Size ?????|Monthly interest rates based on ``interest-rate`` (Example: 1.02).
|service-charge|Decimal|Size ?????|Additional costs of financing (Example: 1.95).
|rate|Decimal|Size ?????|Monthly rate the consumer has to pay (Example: 89.35).
|annual-percentage-rate|Decimal|Size ?????|The annual percentage rate is based on ``service-charge`` and ``interest-rate`` (Example: 12.25).
|number-of-rates|Integer|Size ?????|Number of months to cover the duration of the installment plan.
|amount|Decimal|Size ?????|Price that has to be financed (Example: 1000.00).
|interest-amount|Decimal|Size ?????|Total of all monthly paid interests (Example: 72.09).
|total-amount|Decimal|Size ?????|Total of amount, adding up ``interest-amount`` and ``service-charge`` (Example: 1074.04).
|last-rate|Decimal|Size ?????|Last rate which the consumer has to pay (Example: 89.24).
|payment-first-day|Integer|Size ?????|Calendar day of the first payment.

|===

//// 
With the request fields we always have a corresponding attribute "currency" with amount and rate. Why not in the response?
When I compare the values of installment-calculator-response.amount, installment-calculator-response.installments.installment.details.total-amount, installment-calculator-response.installments.installment.details.amount and installment-calculator-response.installments.installment.details.interest-amount in the sample with the field description at https://confluence.wirecard.sys/display/EE/RatePay+Installment+Calculator I cannot get the things together. Need to talk!
////
...
.XML Calculation by Rate Request
[source,xml,subs=attributes+]
----
include::samples/xml/ratepay-installment_install-calculator-rate_request_success[]
----

.XML Calculation by Rate Response
[source,xml,subs=attributes+]
----
include::samples/xml/ratepay-installment_install-calculator-rate_response_success[]
----

.XML Calculation by Time Request
[source,xml,subs=attributes+]
----
include::samples/xml/ratepay-installment_install-calculator-time_request_success[]
----

.XML Calculation by Time Response
[source,xml,subs=attributes+]
----
include::samples/xml/ratepay-installment_install-calculator-time_response_success[]
----

.JSON Calculation by Rate Request
[source,json,subs=attributes+]
----
include::samples/json/ratepay-installment_install-calculator-rate_request_success[]
----

.JSON Calculation by Rate Response
[source,json,subs=attributes+]
----
include::samples/json/ratepay-installment_install-calculator-rate_response_success[]
----

.JSON Calculation by Time Request
[source,json,subs=attributes+]
----
include::samples/json/ratepay-installment_install-calculator-time_request_success[]
----

.JSON Calculation by Time Response
[source,json,subs=attributes+]
----
include::samples/json/ratepay-installment_install-calculator-time_response_success[]
----
