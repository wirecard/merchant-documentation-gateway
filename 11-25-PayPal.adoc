...
.2+|*Requests/Responses* | Format | XML
                         | Methods | POST
|*IPN Encodement*      2+| Please follow the instructions given at <<GeneralPlatformFeatures_IPN_NotificatonExample_Alternative,Instant Payment Notification>> to set up IPN.
|===
...
For transaction type details which are not given here look at <<AppendixB, Appendix B: Transaction Types>>.
...
|<<API_PayPal_Samples_Workflow_capture-authorization, _capture-authorization_ samples>>
...
1.  1.  the <<GeneralPlatformFeatures_Transactions_Recurring_Periodic, periodic type>> part must be either "recurring" or "installment"
2.  the <<GeneralPlatformFeatures_Transactions_Recurring_Sequence, sequence type>> must be "first".
** amount = "0.00"
...
See
<<API_PayPal_Samples_GeneralSamples_debit, _debit_ samples>> for details.
