...
|custom-field/[@http://zapp.in[zapp.in].MerchantRtnStrng] |M |O |O |String    |  |Merchant Return String to redirect the Consumer from the
                                                                                  Mobile Banking App to the Merchant’s browser or App.
|custom-field/[@http://zapp.in[zapp.in].TxType]           |M |O |O |String    |  |Type of Transaction.
|custom-field/[@http://zapp.in[zapp.in].DeliveryType]     |M |O |O |String    |  |The delivery type chosen for the delivery of purchased goods.
...
|requested-amount[@currency]                             |M |M |M |String    |3 |Zapp supports *only GBP.*
|custom-field/[@zhttp://app.in[app.in].RefundReasonType] |M |O |O |String    |  |Used to complete the reason for the Refund, and when
                                                                                 applicable will indicate whether a Refund is the result of a Query or Dispute case.
|custom-field/[@http://zapp.in[zapp.in].RefundMethod]    |M |O |O |String    |  |The method of Refund that will be used by Wirecard to make the refund payment
|custom-field/[@zapp.inout.MerchantRefundRef]            |O |O |O |String    |  |Merchant assigned reference for the Refund.
|custom-field/[@http://zapp.in[zapp.in].CaseRefId]       |O |O |O |String    |  |Case Reference Id generated by Query/ZDMS to be populated when the Refund is a result of a Query or Dispute. The
...
|custom-field/[@zhttp://app.in/[app.in].RefundReasonType]     |O |String    |      |Used to complete the reason for the Refund, and when applicable will
                                                                                  indicate whether a Refund is the result of a Query or Dispute case.
|custom-field/[@http://zapp.in/[zapp.in].RefundMethod]        |O |String    |      |The method of Refund that will be used by Wirecard to make the refund payment
|custom-field/[@zapp.inout.MerchantRefundRef]                 |O |String    |      |Merchant assigned reference for the Refund.
|custom-field/[@http://zapp.in/[zapp.in].CaseRefId]           |O |String    |      |Case Reference Id generated by Query/ZDMS to be populated when the Refund is
