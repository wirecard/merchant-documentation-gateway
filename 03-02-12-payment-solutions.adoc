...
[mermaid,PPv1_PaymentSolutions_InvoiceviaEmail_Workflow,svg,subs=attributes+]
----
sequenceDiagram
    participant A as Consumer on HPP
    participant B as Consumer
    participant C as Merchant
        activate C
        C -> B: Logs in to the portal.
        C -> B: Selects "Invoicing By Email".
        C -> B: Provides payment information.
            opt 1
                C -> B: Can optionally either upload an invoice or other attachments.
            end
        C -> B: Clicks "Generate Payment URL" button.
            alt 1
                C ->> B: Clicks "Send" button.
                    opt 1a
                        C -> B: URL can be sent via email.
                    end
            else 2
                C -> B: Copies Payment URL to clipboard.
                    opt 2a
                        C -> B: URL must be sent via other device/unit.
                    end
            end
        C ->> B: Sends email with Payment URL.
        deactivate C
        activate B
        B ->> A: Clicks on Payment URL.
        deactivate B
        activate A
        A -> B: Continues payment process.
        deactivate A
----
