...
[#PP_PaymentSolutions_PaybyLink_Intro]
====== Introduction
...
The hyperlink redirects the consumer on a credit card
form within HPP in order to supply customer’s card data. Additionally,
...
format consisted of combined date and time in UTC:
...
REST API, specifically _paymentmethods_ endpoint without entering card
...
. Redirect URL must be specified. If merchants hasn't been configured
on Merchant level configuration, Redirect URL must be send via request.
