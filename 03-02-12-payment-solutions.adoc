...
sequenceDiagram
    ...
    participant C as Merchant
        activate C
        C -->> C: logs in to the portal
        C -->> C: selects "Invoicing By Email"
        C -->> C: provides payment information
            alt either
              C -->> C: uploads invoice
            else or
              C -->> C: uploads other attachments
            end
        C -->> C: clicks "Generate Payment URL"
            alt sends URL either
                C ->> B: via email
            else or copies URL and sends
                C ->> B: via other device/unit
            end
    	deactivate C
        activate B
        B ->> A: clicks on URL
