...
| <<CreditCard_TransactionTypes_Authorization, _prauthorization_>> | Reserves funds from the cardholder's account. Typically, the limit ranges from three to thirty days to conduct a _capture-preauthorization_, depending on the acquirer and card brand.
...
| <<CreditCard_TransactionTypes_Purchase_SendingData_ReferencingPurchase, _referenced-purchase_>> | Takes funds from the cardholder's account. Identical to a _purchase_ except for the fact that it refers to a previous _purchase_ in the context of recurring transactions.
...
given in the <<CreditCard_TransactionTypes_Purchase_SendingData_UsingCardData, Request Header box>> at each request sample.
