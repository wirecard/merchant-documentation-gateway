 ...
 */
PMUtil.stfuGetJsonFromFile = function (file, strict = false) {
    var fileContents;
    try {
...
PMUtil.createTestCredentialsTables = function (RequestResponseIndex) {
    ...
    for (var k in RequestResponseIndex) {
        ...
        const basename = PMUtil.camelCase(folderPathString); // e.g. "klarna-install"
        /*
...
PMUtil.writeAdocSummary = function (RequestResponseIndex) {
    ...
    for (var t in RequestResponseIndex) {
        ...
        const basename = PMUtil.camelCase(t);
...
`;
        ...
        try {
            ...
            if (numSuccessfulRequests > 0 && numSuccessfulRequests >= numExistingRequests) { // if in a previous run there were more successful requests thant in the current, do not write the adoc
                ...
                process.stderr.write(PMUtil.styleText('WRITTEN: ', 'green') + adocFilename +
                    ' (' + numSuccessfulRequests + ' >= ' + numExistingRequests + ')' + "\n");
            } else {
                process.stderr.write(PMUtil.styleText('SKIPPED: ', 'red') + adocFilename + ' (' +
...
PMUtil.styleText = function (text, style, type = 'fg') {
