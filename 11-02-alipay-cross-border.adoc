...
|String 
|15 
|The periodic type value can only be _recurring_.

|periodic/sequence-type 
|M 
|String 
|15 
|The sequence type value can be _first_, _recurring_ or _final_ depending on transaction type.
...
- Sales of goods +
- Others (for example, mobile data service recharge, airport pick up service etc.) 
//-

If more than one type is used, separate the values with a vertical bar.

|hotel_name
|M
|String
|
|The hotel name. It consists of numbers, letters, spaces, and special characters including ``,.<>()[]/-,.``. +
If more than one hotel name exists, separate the values with a vertical bar. + 

*Include this field only when the business_type is set to _1_ (_Hotel_).*

|check_in_time
|M
|Date
|
|The check-in time. Format: ``yyyy-MM-dd``. Timezone: GMT +8. 
...
|Date
|
|The check-out time. Format: ``yyyy-MM-dd``. Timezone: GMT +8. 
...
|String
|
|The flight number. If a flight transfer exists, separate the flight numbers with a vertical bar. 

*Include this field only when the business_type is set to _2_ (_AIR_).*

|departure_time
|M
|Date
|
|The departure time. Format: ``yyyy-MM-dd HH:mm``. Timezone: GMT +8. +
If flight transfers exist, separate the time values with a vertical bar. 
...
|String
|
|*Include this field when the business_type is set to _3_ (_Overseas study consulting_).*

|goods_info
|M
|String
|
|Goods information including SKU names and corresponding quantities. Format: ``SKU_name^quantity``. For example: ``pencil^2, eraser^5, iPhone XS 256G^1``. + 
If more than one type of goods exists, separate the values with a vertical bar. 

*Include this field only when the business_type is set to _4_ (_Sales of goods_).*

|total_quantity
|M
|Number
|
|The total quantity of goods in one order. 

*Include this field only when the business_type is _4_ (_Sales of goods_).*

|other_business_type
|M
|String
|
|*Include this field when the business_type is _5_ (_Others_).*
...
When using HPP/EPP _{payment-gateway}_ detects the device in use
...
----
include::samples/xml/alipay-xborder_debit_request_success_en.xml[]
----
...
[source,xml,subs=attributes+]
----
include::samples/xml/alipay-xborder_debit_response_success_en.xml[]
----
...
[source,xml,subs=attributes+]
----
include::samples/xml/alipay-xborder_refund-debit_request_success.xml[]
----
...
[source,xml,subs=attributes+]
----
include::samples/xml/alipay-xborder_refund-debit_response_success_en_a9bbf42e.xml[]
----
...
[source,xml,subs=attributes+]
----
include::samples/xml/alipay-xborder_authorization-only_request_success_recurring_first.xml[]
----
...
[source,xml,subs=attributes+]
----
include::samples/xml/alipay-xborder_get-url_response_success_recurring_first.xml[]
----
...
[source,xml,subs=attributes+]
----
include::samples/xml/alipay-xborder_debit_request_success_recurring_recurring_649cb805.xml[]
----
...
[source,xml,subs=attributes+]
----
include::samples/xml/alipay-xborder_get-url_response_success_en_recurring_recurring_649cb805.xml[]
----
...
[source,xml,subs=attributes+]
----
include::samples/xml/alipay-xborder_authorization-only_request_success_recurring_final_f704f0a9.xml[]
----
...
[source,xml,subs=attributes+]
----
include::samples/xml/alipay-xborder_authorization-only_response_success_recurring_final_f704f0a9.xml[]
----
