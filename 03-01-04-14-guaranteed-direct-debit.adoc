...
_Guaranteed Direct Debit_ allows the consumer to pay directly from their bank account. The invoicing, direct debit and collection process (dunning) is handled by {payment-provider-name}. Guaranteed Direct Debit ensures that merchants receive the payment amount from {payment-provider-name} prior to settlement with consumers.
