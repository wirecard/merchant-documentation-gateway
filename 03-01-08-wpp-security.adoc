...
processes, see <<PaymentPageSolutions_WPP_HPP_Integration, HPP Integration>>,
<<PaymentPageSolutions_WPP_EPP_Integration, EPP Integration>> and
<<WPP_Seamless_Integration, Integrating WPP in Seamless Mode>>.
...
- the field entries ``response-base 64``, ``response-signature-base64`` and ``response-signature-algorithm`` sent
...
NOTE: The signature is only used with requests sent to the Wirecard Payment Gateway endpoint, e.g.
