...
NOTE: We recommend sending the optional fields, such as the `account-holder` phone numbers and the `shipping.address` information. If you submit this information, it is more likely that the Issuer authenticates the consumer without requesting <<CreditCard_PSD2_SCA, Strong Customer Authentication>>.
