...
Once _{payment-gateway-abbr}_ has received a notification from _PayPal_ about the final
...
_authorization-only_ (records data at _PayPal_ and _{payment-gateway-abbr}_ for later
...
transaction is created in the database. Once _{payment-gateway-abbr}_ has received a
...
|transaction-id | |M |M |String |36 |The Transaction ID is the unique identifier for a transaction. It is generated automatically.
...
must be communicated to {payment-provider-name} to submit payments.
...
corresponding merchant accounts of the processing user by <<ContactUs, Merchant Support>>.
...
[source,xml,subs=attributes+],subs=attributes+
----
