...
image::images/11-30-Sepa/SepaLogoEN.jpg[SEPA Logo, 200, align="right"]
...
_Wirecard Payment Processing Gateway_ allows merchants to void SEPA _Direct Debit_
...
time the transaction request has been received by the _Wirecard Payment Processing Gateway_
...
These supplementary payments enter the _Wirecard Payment Processing Gateway_ as
...
in the descriptor of his transfer, the _Wirecard Payment Processing Gateway_ will
...
reconciliation files provided by Wirecard Bank and sent to the _Wirecard Payment Processing Gateway._ The transaction types _debit-return_ and
_credit-return_ are then initiated by the _Wirecard Payment Processing Gateway_. It is not possible for a
merchant to request these transaction types.

If possible, a _debit-return_ or _credit-return_ is matched by the _Wirecard Payment Processing Gateway (WPPG)_ to its original debit/credit, based on the provider transaction
...
image:images/11-30-Sepa/3704722.png[SEPA Workflow]

. _Consumer's bank sends a debit-return_ or _credit-return_ to
Wirecard Bank.
. Wirecard Bank forwards the transaction to the WPPG.
. WPPG receives the transaction.
. WPPG uses the PTRID to find an initial parent _debit_ or parent
_credit_ transaction.
//-

.. If successful, WPPG maps the _debit-return_ to _a debit_ or
_credit-return_ to a _credit._
.. if not successful, WPPG creates _a debit-return_ or _credit-return_
without reference.
. WPPG sends a notification about the _debit-return_ or _credit-return_
to the merchant.
//-
...
The _Wirecard Payment Processing Gateway_ validates the consumer's bank data, to
...
_Wirecard Payment Processing Gateway_ validates the consumer's IBAN in the
following manner:
...
_Wirecard Payment Processing Gateway_ validates the consumer's BIC in the following
manner:
