...
The consumer pays by invoice upon receipt of the goods.  {payment-provider-name} pays the merchants the order amounts in advance of settlement with consumers. The invoicing and collection process (including dunning) is also handled by {payment-provider-name}.
...
Every merchant needs to follow test process chains successfully in a test environment to get an approval to go live. Wirecard checks and approves the frontend/checkout and invoices (if generated and sent out by the merchant).
image:images/03-01-04-12-guaranteed-invoice/arrow.png[click here, width=18] <<GuaranteedInvoice_ProcessChain, More information on the process chain.>> 
...
[NOTE]
====
. <<GuaranteedInvoice_BasketItemsDetails, All Shopping basket items>>  must be specified in every request.
...
.6+|order-item .2+e|name |description |String |Required | |Name of the item in the shopping basket.
