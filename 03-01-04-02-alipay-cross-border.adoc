...
This is a reference page for _Alipay Cross-border._ Here you'll find all
...
image:images/03-01-04-02-alipay-cross-border/alipay_crossborder_enterdata.png[Alipay Cross-border login form,width=750]
...
2+|transaction-type |String |The requested transaction type. For Alipay Cross-border payments, transaction-type must be set to _debit_.
...
state either _success_ or _failed._ This information is returned in the response
only.
2+|transaction-type |String |The requested transaction type. For Alipay Cross-border payments, transaction-type must be set to _debit_.
2+|cancel-redirect-url |String a|The URL to which the consumer is redirected after having cancelled a payment,
