...
[cols="15,85"]
|===
| ``three-d/pareq`` | Base64-encoded request message created for cards participating in the 3D Secure program. + The ``PAReq`` is returned by the issuer's ACS via the VISA or Mastercard directory to the {payment-gateway} and from there passed on to the merchant.
