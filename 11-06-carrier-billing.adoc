...
|_debit_ | <<CarrierBilling_Samples_Debit, _debit_ sample>>
...
===== Additional Test Credentials on __carrier billing's_ Environment
...
.  The merchant sends a payment request to _Wirecard Payment Processing Gateway_.
...
.  On the _Payment Page_.
.  Consumers select _carrier billing_.
.  _Wirecard Payment Processing Gateway_ redirects consumers to provider's page.
