...
image::images/11-25-paysafecard/paysafecard_logo.png[paysafecard Logo, width=200, float="right", align="right"]
...
the endpoint of WPPG.
. WPPG sends a response to the merchant with generated URL
...
. _paysafecard_ sends the authorization status to WPPG.
...
. WPPG redirects consumer from website to _paysafecard_ landing page.
...
. _paysafecard_ sends the transaction status to WPPG.
. WPPG sends a Success/Failed notification to the merchant.
