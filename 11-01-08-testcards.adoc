...
Merchant->>{payment-gateway-abbr}: sends a check-enrollment request to {payment-gateway-abbr}.
{payment-gateway-abbr}->>{payment-gateway-abbr}: Checks card data.
Note right of Merchant: check-enrollment <br/>response contains <br/>pareq.
{payment-gateway-abbr}->>Merchant: sends a check-enrollment response to merchant.
...
Note right of Merchant: check-payer- <br/>response request <br/>contains pares.
Merchant->>{payment-gateway-abbr}: Sends check-payer-response request to {payment-gateway-abbr}.
{payment-gateway-abbr}->>{payment-gateway-abbr}: Accepts card in 3D-Secure payment process.
{payment-gateway-abbr}->>Merchant: Sends check-payer-response response to merchant.
----
...
sequenceDiagram
    ...
    participant Issuer
Consumer->>Merchant: Initiates a 3D secure Credit Card payment.
Note right of Merchant: Contains card data (card number, CVV, etc).
Merchant->>{payment-gateway-abbr}: sends a check-enrollment request to {payment-gateway-abbr}.
{payment-gateway-abbr}->>{payment-gateway-abbr}: Checks card data.
Note right of Merchant: check-enrollment response contains pareq.
{payment-gateway-abbr}->>Merchant: sends a check-enrollment response to merchant.
Note right of Merchant: Receiver is taken from acs-url.
Merchant->>Issuer: sends pareq to issuer.
Issuer->>Consumer: Shows authentication UI to consumer.
Consumer->>Consumer: Authenticates card data.
Consumer->>Issuer: Issuer receives authentication result.
Issuer->>Merchant: Sends pares to merchant.
Note right of Merchant: check-payer-response request contains pares.
Merchant->>{payment-gateway-abbr}: Sends check-payer-response request to {payment-gateway-abbr}.
{payment-gateway-abbr}->>{payment-gateway-abbr}: Accepts card in 3D-Secure payment process.
{payment-gateway-abbr}->>Merchant: Sends check-payer-response response to merchant.
----
...
. {payment-gateway-abbr} accepts the card in 3D Secure payment process.
