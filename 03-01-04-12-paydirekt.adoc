...
For _paydirekt_ we provide samples for the available transaction types 
<<PPv2_paydirekt_TransactionType_debitauth, _authorization_>> and 
<<PPv2_{payment-method-name}_TransactionType_debit, _debit_>>  
...
This table shows the  _paydirekt_ payment mode, and the countries and currencies
...
With _paydirect_ you can select <<PPv2_paydirekt_TransactionType_debitauth, _authorization_>> and <<PPv2_paydirekt_TransactionType_debit, _debit_>>.
...
|Required 
|7,2 
|The full amount that is requested/contested in a transaction. The amount of the decimal place is dependent of the currency. Minimum is ``0.01``. Maximum is ``50000``. Use ``.`` (decimal point) as the separator.
...
|Required 
|22 
|The requested transaction type. For _paydirekt_ payments, the transaction-type is either ``authorization`` and ``debit``.
...
3+|transaction-type 
|String 
|The requested transaction type. For _paydirekt_ payments, the transaction-type is either ``authorization`` or ``debit``.
...
Hi Karin, here we already have a reference to a full list of paydirekt fields. Why do we need another one in the include file below (which I just extracted from the NVP-JSON merge file)? 
This morning, when we discussed it, I was not aware of this section. Under these circumstances, I think, we do not need this extra include file.
...
WARNING: Check the <<paydirekt_TransactionTypes, REST API paydirekt specification>> for details on _paydirekt_-specific post-processing operations.
