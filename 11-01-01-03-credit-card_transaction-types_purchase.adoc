...
<<CreditCard_TransactionTypes_Purchase_SendingData_ReferencingTransactions, referencing a _purchase_ transaction>>.
...
Read how a
<<CreditCard_PaymentFeatures_Introduction, token replaces the clear credit card data>>.
...
- a <<CreditCard_TransactionTypes_Purchase_SendingData_VoidPurchase, _void-purchase_>> (details see <<CreditCard_TransactionTypes__VoidRefund_Void, void>>).
- a <<CreditCard_TransactionTypes_Purchase_SendingData_RefundPurchase, _refund-purchase_>> (details see <<CreditCard_TransactionTypes__VoidRefund_Refund, refund>>).
//-
...
[#CreditCard_TransactionTypes_Purchase_SendingData_ReferencingPurchaseTransactions_PurchaseRequest]
...
[#CreditCard_TransactionTypes_Purchase_SendingData_ReferencingPurchaseTransactions_PurchaseResponse]
...
[#CreditCard_TransactionTypes_Purchase_SendingData_ReferencingPurchaseTransactions_ReferencedPurchaseRequest]
...
[[purchase-referenced-purchaseResponse(recurring/recurring)]]
___referenced-__purchase_ Response (recurring/recurring)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

*Fields*

We provide detailed descriptions of
all https://document-center.wirecard.com/pages/viewpage.action?pageId=3703633[_credit
card_ fields].
...
[[purchase-referenced-purchaseRequest(recurring/final)]]
...
[[purchase-referenced-purchaseResponse(recurring/final)]]
___referenced-__purchase_ Response (recurring/final)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
A __ _void-purchase_ must reference a successful
https://document-center.wirecard.com/display/PTD/purchase#purchase-purchaseUsingaToken[_purchase_
response].

A _void-purchase_ shall be used only, if the payment was processed in an
online shop and not at a POS.

We only list field descriptions for requests and responses.
Notifications follow the general structure described in
https://document-center.wirecard.com/display/PTD/General+Platform+Features#GeneralPlatformFeatures-CreditCardPayment[General
Platform Features]
...
A ___refund-purchase___ must reference a
successful __https://document-center.wirecard.com/display/PTD/purchase#purchase-purchaseUsingaToken[_purchase_]__
https://confluence.wirecard.sys/display/PTD/purchase#purchase-Response.1[response].
...
_void-refund-purchase (details see_
https://document-center.wirecard.com/display/PTD/Transaction+Types#TransactionTypes-void[void]_)._

[[purchase-void-refund-purchase]]
...
link:attachments/5406747/5406749.png[CC_purchase_complete_sequence_StartEnd.png]
(image/png) +
image:images/icons/bullet_blue.gif[image,width=8,height=8]
link:attachments/5406747/5406750.png[CC_purchaseReferencing_flow.png]
(image/png) +
