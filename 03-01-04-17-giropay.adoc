...
If IBAN was sent in the request then this value should be displayed/prefilled on the consumer bank. But yes, it depends on the consumer bank.
...
If you enter the BIC, the redirect to the _{payment-method-name}_ is initiated. If not, the WPP v2 displays a field/sandbox for entering the BIC.
...
This field is returned in the response if the communication to the provider was made. giropay has returned them as part of payment notification to EE. 
So, once redirect to Giropay is made, they are returned in the response.
