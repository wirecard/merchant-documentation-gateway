...
[mermaid, PPv2_giropay_Workflow,svg,subs=attributes+]
----
sequenceDiagram
    participant Consumer
    participant Merchant
    participant {payment-gateway-abbr}
    participant Consumer's Bank
Consumer->>Consumer: adds items to shopping basket
Consumer->>Merchant: selects payment method giropay
Merchant->>{payment-gateway-abbr}: forwards information
{payment-gateway-abbr}->>{payment-gateway-abbr}: processes and validates the transaction
{payment-gateway-abbr}->>Consumer's Bank: sends the request to giropay
Consumer's Bank->>{payment-gateway-abbr}: verifies payment
{payment-gateway-abbr}->>{payment-gateway-abbr}: processes the response 
{payment-gateway-abbr}->>Consumer: redirects the consumer to the provided online banking URL
Consumer->>Consumer's Bank: verifies the payment within the personal online basket system
Consumer's Bank->>Consumer's Bank: transfers funds from consumer's bank to merchant's bank
Consumer's Bank->>{payment-gateway-abbr}: sends payment notification
{payment-gateway-abbr}->>Merchant: sends payment notification
----

. Consumer adds items to the shopping basket.
. Consumer selects payment method _{payment-method-name}_.
. The merchant sends a request information to _{payment-gateway-abbr}_.
. _{payment-gateway-abbr}_ processes and validates the transaction.
. _{payment-gateway-abbr}_ forwards the request to the consumer's bank (giropay).
. Consumer's bank (giropay) verifies payment and sends verification to _{payment-gateway-abbr}_.
.. verification contains consumer's online banking URL
. _{payment-gateway-abbr}_ processes the response.
. _{payment-gateway-abbr}_ redirects the consumer to the provided online banking URL.
. Consumer verifies the payment within the personal online banking system.
. Consumer's bank (giropay) transfers funds from the consumer’s bank account to the merchant’s bank account.
. Consumer's bank (giropay) sends a payment notification to the _{payment-gateway-abbr}_.
. _{payment-gateway-abbr}_ processes the notification and sends the payment debit notification to the merchant.
...
|===
|Authorization
a| Use the username and password as given in your {payment-provider-name} contract to _Base64_ encode the authorization.
...
| M 
| 256 
| The name of the payment method used. Set this value to _{payment-method-name}_.
...
include::samples/json/WPP_giropayTransactionTypeDebit_3ParseAndProcessThePaymentResponseDecodedPaymentResponse.json[]
...
2+| provider_transaction_id
| String
| Unique identifier of a provider transaction, typically generated by the provider.
|===
