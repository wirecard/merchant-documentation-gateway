...
ifdef::env-nova[]
It combines the payment methods Credit Card (with/without 3D
secure), iDEAL, SEPA, paypal, Wirecard Voucher and eps-Überweisung including their
related options in one solution.
endif::[]

//// 
vhauss: I modified the content from the preceding "ifdef::env-wirecard" section and put it in a "ifdef::env-nova" section, because the description partly applies for NOVA and I think we cannot exclude it completely. The complete exclusion applies for our Whitelabel partners like Paydoo perfectly as they only offer Credit Card.
////
