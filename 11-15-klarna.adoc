...
It is only possible to extend the reservation expiry date after it has
already expired.
...
A risk check can be performed with every update, if the total order
...
A partial captured reservation still can be cancelled. In this case only
...
In case of the payment instructions, the merchant is responsible to send
...
In case of uneven amounts the calculation and rounding of amounts should
...
If the decrease of the voucher is based on the overall basket
...
The merchant requires the Klarna _Merchant ID_ in order to be able to
...
The _Installment Calculator_ request is being sent to
