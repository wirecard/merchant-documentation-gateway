...
the final status of a payment as soon as the _{payment-gateway}_ receives it from the
...
Once the _{payment-gateway}_ has received a notification from the _carrier-billing_
...
unique identifier for a transaction. It is generated automatically.
