...
. *Check-enrollment* (full amount): This is the initial request and initiates the payment session. New 3D Secure 2 fields can be found in the <<CreditCard_3DS2_Fields, 3D Secure 2 field table>> and are included in the <<Appendix_Xml, REST API payment XSD>>.
+
CAUTION: For 3D Secure 2 consumer-initiated  (CI) one-click checkout,
include the new field ``three-d/version`` and set its value
to ``2.1``. Set ``challenge-indicator`` to ``04`` and flag the transaction
as consumer-initiated in ``periodic-type/ci``.
+
[%autowidth]
|===
|XML |Fields
...
====== Option 1: One-Step Subsequent Payment with *Authentication*
...
NOTE: It is not required to include the ``periodic-type`` set  to ``ci`` and
...
====== Option 2: Two-Step Subsequent Payment with *Authentication*
...
===== Storing Credit Card Credentials Scenario and Subsequent Consumer-Initiated (CI) One-Click Checkout
...
CAUTION: For 3D Secure 2 transactions, include the new field
``three-d/version`` and set its value to ``2.1``. In addition, include
``periodic-type`` set to ``ci`` and ``challenge-indicator`` set to ``04``.
+
[%autowidth]
|===
|XML 2+|Fields
...
NOTE: It is not required to include the ``periodic-type`` set  to ``ci``.
+
[%autowidth]
|===
|XML 2+|Fields
...
====== Option 2: Storing Card Credentials (Reserve and Void Amount) with *Authentication*
...
NOTE: It is not required to include ``periodic-type`` or ``three-d``.
...
====== One-Step Subsequent payment with *Authentication*
...
NOTE: It is not required to include the ``parent-transaction-id`` or the
``challenge-indicator`` set to ``04``. In addition, ``token-id`` can be used
