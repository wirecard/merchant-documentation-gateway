...
----
include::samples/json/PaymentPageSolutions_WPP_EPP_IntegrationExampleCreate_CreateAPaymentSessionInitialRequest.json[]
----
...
[source,json,subs=attributes+]
----
include::samples/json/PaymentPageSolutions_WPP_EPP_IntegrationExampleEmbed_ResponseToAuthorizationRequest.json[]
----

[#PaymentPageSolutions_PPv2_EPP_Integration_Example_Embed_WPPEmbeddedPayUrl]
*Embedding the Payment Page in your Checkout Page Using ``{payment-page-function-short}.embeddedPayUrlembeddedPayUrl``*
...
response to the ``{payment-page-function-short}.embeddedPayUrlembeddedPayUrl`` function and call it to render the
payment page.

. Submit the *initial payment request* on the backend.

. Call the ``{payment-page-function-short}.embeddedPayUrlembeddedPayUrl(payment-redirect-url)`` function
in your HTML code to render the new pop-up window.
+
[source,html,subs=attributes+]
----
<script type="text/javascript">{payment-page-function-short}.embeddedPayUrlembeddedPayUrl("https://{payment-redirect-url-hostname}/?wPaymentToken=f0c0e5b3-23ad-4cb4-abca-ed80a0e770e7")</script>
----
...
[source,json,subs=attributes+]
----
include::samples/json/PaymentPageSolutions_WPP_EPP_IntegrationExampleParse_ParseAndProcessThePaymentResponseDecodedPaymentResponse.json[]
----
