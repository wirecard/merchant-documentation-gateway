...
*Embedding the Payment Page in your Checkout Page Using ``PaymentPage.embeddedPayUrl``*
...
response to the ``PaymentPage.embeddedPayUrl`` function and call it to render the
payment page.

. Submit the *initial payment request* on the backend.

. Call the ``PaymentPage.embeddedPayUrl(payment-redirect-url)`` function
in your HTML code to render the new pop-up window.
+
[source,html,subs=attributes+]
----
<script type="text/javascript">PaymentPage.embeddedPayUrl("https://{payment-redirect-url-hostname}/?wPaymentToken=f0c0e5b3-23ad-4cb4-abca-ed80a0e770e7")</script>
----
