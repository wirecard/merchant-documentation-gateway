...
``02`` = Credit. +
``03``= Debit. +
The cardinality depends on the use case. The field is required: 
...
|Numeric
|18,2
|For prepaid or gift card purchase. 
//KKS: Description provided in ticket is not sufficient. 
...
|String
|2048
|This is the HTTP Accept Header as retrieved from the consumer’s browser in the HTTP request. If is longer than 2048 it must be truncated. It is strongly recommended to provide this field to prevent rejection from the ACS server.
...
``05`` = Account
//KKS: Supposedly required for 3DS2.2 only. Does that mean it's mandatory for 3DS2.2, or that it's available for 2.2?
|===
