...
[mermaid,MultiPaymentApi_Workflow,svg,subs=attributes+]
----
sequenceDiagram
    participant A as Merchant (Shop)
    participant B as Elastic Engine
    participant C as Payment Provider
        A ->> B: Multipayment Request
        activate B
        B -> C: Validate Payment Request
        B -> C: Generate Single payment request for each payment method
        B ->> C: Send request for a single payment method
        deactivate B
        activate C
        C -> B: Process the payment request
        C ->> B: Payment Response
        deactivate C
        C -> B: Payment Successful?
        activate B
            alt Yes
                B -> C: Pending Payment Request?
                    alt Yes
                        B ->> C: Send single payment request
        deactivate B
                        activate C
                        C -> B: Process the payment request
                        C ->> B: Payment Response
                ...
                B ->> C: Rollback previously successful payment transactions
        activate C
                C -> B: Cancel the previous payment transaction
            end
        C ->> B: Payment Response
        deactivate C
        activate B
        B -> A: Build Response
        B ->> A: Multipayment Response
        deactivate B
----
...
----
include::samples/xml/MultiPaymentApiSamples_RequestPurchaseCreditCardAnd2Vouchers.xml[]
----

.Response Purchase Successful
[source,xml,subs=attributes+]
----
include::samples/xml/MultiPaymentApiSamples_ResponsePurchaseSuccessful.xml[]
----

.Request Void-Refund-Purchase
[source,xml,subs=attributes+]
----
include::samples/xml/MultiPaymentApiSamples_RequestVoidRefundPurchase.xml[]
----

.Response Void-Refund-Purchase – Voucher 2 failed, Voucher 1 Rolled Back
[source,xml,subs=attributes+]
----
include::samples/xml/MultiPaymentApiSamples_RVRPV2fV1RB.xml[]
----
