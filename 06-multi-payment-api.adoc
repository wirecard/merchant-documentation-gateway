...
sequenceDiagram
    ...
    participant C as Payment Provider
        A ->> B: multi payment request
        activate B
        B -->> B: validates request
        B -->> B: generates individual request for each payment method
        B ->> C: sends individual request
        deactivate B
        activate C
        C -->> C: processes request
        C ->> B: sends response
        deactivate C
        B -->> B: Payment successful?
        activate B
            alt yes
                B -->> B: Pending payment request?
                    alt yes
                        B ->> C: sends individual request
        deactivate B
                        activate C
                ...
                B ->> C: rolls back previously successful transactions
        activate C
                C -->> C: cancels previous transaction
            end
        C ->> B: sends response
        deactivate C
        activate B
        B -->> B: builds response
        B ->> A: multi payment response
        deactivate B
----
