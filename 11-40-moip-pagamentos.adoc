...
configured for _moip_ specifically, otherwise it won't be able to
...
the transaction is processed automatically by _Wirecard Payment
Gateway_.
...
*Expiration Date* needs to be in future and *CVV* can be anything with 3
...
The *token* & *keys* will be provided afterwards to the merchant support
...
. Merchant sends _purchase_ request to WPG.
. WPG returns _pending-purchase_ back to merchant.
. Moip acknowledges the payment to WPG within 48 hours.
. WPG sends a final notification to merchant.
. The payment is done.
...
|===
.2+h|Field 3+h|Transaction Process (REST API) 3+h|Transaction Process (Hosted Payment Page) .2+h|Data .2+h|Size .2+h|Description
...
It is optional for _Capture & Void_ requests (only the full amount can
...
|avs-code | |O |O | | | |String |36 |This is the result of address's validation.
...
|email |M |M |M |M |M |M |String |32 |The is email address of the consumer.
...
.2+h| Field 3+h|Transaction Process (REST API) 3+h|Transaction Process (Hosted Payment Page) .2+h|Data .2+h|Size .2+h|Description
h|Request h|Response h|Notification h|Request h|Response h|Notification
|account-number |M | | | | | |String |36 |This is the card account number of the consumer. If is mandatory if
...
|expiration-year |M |O | | | | |Numeric |4 |This is the card's expiration year of the onsumer. If this field is
...
[#MoipPagamentos_XMLamples_RestApi_payments_purchase]
====== _purchase_

This _purchase_ sample set can be also used with the transaction type
...
===== Use REST API addressing the endpoint _/paymentsmethods_
...
[#MoipPagamentos_Samples_RestApi_paymentmethods_purchase]
====== _purchase_

This _purchase_ request can be also used with the transaction type
_authorization_. *Contains mandatory fields only*.
