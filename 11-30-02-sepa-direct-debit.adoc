...
[[SEPADirectDebit-Referencingtoanauthorization]]
...
It is also possible to reference a
https://document-center.wirecard.com/display/PTD/SEPA+Credit+Transfer#SEPACreditTransfer-refund[refund] to
an _authorization_ if the merchant's business flow requires this.

[[SEPADirectDebit-ReferencingtoaFailedauthorization]]
...
[[SEPADirectDebit-TestCredentials]]
Test Credentials
----------------
...
[[SEPADirectDebit-Workflow]]
Workflow
--------

image:attachments/3703429/3703426.png[image,height=250]

* The transaction type within the request must be _debit or
pending-_debit_._
...
_debit-return_ reasons, please refer to
...
https://document-center.wirecard.com/display/PTD/Fields[API field list]
or the request example for the fields required in a
https://document-center.wirecard.com/display/PTD/SEPA+Direct+Debit#SEPADirectDebit-DirectDebit.1[Direct
Debit] transaction.
...
Can be used with the transaction types _debit_ and _pending-debit_. When
...
This is the Bank Identifier Code of the end-consumer' bank. In SEPA Area
where
https://document-center.wirecard.com/display/PTD/SEPA#SEPA-SEPAIBANOnly[IBAN
...
https://confluence.wirecard.sys/display/PTD/SEPA+Direct+Debit#SEPADirectDebit-MandateID[mandate.mandate-id]
...
https://confluence.wirecard.sys/display/PTD/SEPA+Direct+Debit#SEPADirectDebit-MandateSignatureDate[mandate.signature-date]
...
https://confluence.wirecard.sys/display/PTD/SEPA+Direct+Debit#SEPADirectDebit-CreditorID[creditor-id]
...
Identifies and allows a merchant to process SEPA _Direct
Debit_ transactions.
...
https://confluence.wirecard.sys/display/PTD/SEPA+Direct+Debit#SEPADirectDebit-ProviderTransactionReferenceID[provider-transaction-reference-id]
...
[[SEPADirectDebit-RecurringTransaction.1]]
Recurring Transaction
~~~~~~~~~~~~~~~~~~~~~

The following fields are required *additionally* for a _Recurring_
transaction.
...
Transaction ID of the first transaction of a payment. It is mandatory
...
Indicates how and why a Payment occurs more than once. Only two possible
values: 'recurring' or "Installment’

periodic.periodic-type.sequence-type
...
[[SEPADirectDebit-authorization.1]]
_authorization_
~~~~~~~~~~~~~~~
...
notifications are the same as the REST API Fields. Please refer to the
https://document-center.wirecard.com/display/PTD/Fields[API field list]
or the request example for the fields required in an
_https://document-center.wirecard.com/display/PTD/SEPA+Direct+Debit#SEPADirectDebit-authorization.2[authorization]_
transaction.
...
[[SEPADirectDebit-SEPADirectDebitSpecificFields]]
...
perform SEPA _Direct Debits_. Depending on the originating country, the
...
The countries currently validated by the _Wirecard Payment Gateway_ are
...
[[SEPADirectDebit-ErrorMessagesincaseofanincorrectlysubmittedCreditorID]]
...
Codes and Transaction Statuses]
...
The mandate ID may *not* be generated by the _Wirecard Payment Gateway_.
It is solely the responsibility of the merchant.

[[SEPADirectDebit-MandateSignatureDate]]
Mandate Signature Date
^^^^^^^^^^^^^^^^^^^^^^
...
[[SEPADirectDebit-ProviderTransactionReferenceID]]
Provider Transaction Reference ID
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
lifecycle of a payment is identifiable. +
Wirecard generates this ID for the merchant.

[[SEPADirectDebit-DueDate]]
Due Date
^^^^^^^^
...
Wirecard cannot replace an incorrect due date sent in by the merchant
...
[[SEPADirectDebit-CalculatingtheDueDate]]
...
read chapter
https://document-center.wirecard.com/display/PTD/SEPA+Direct+Debit#SEPADirectDebit-RecurringTransaction[SEPA
Direct Debit Recurring Transaction].

[[SEPADirectDebit-DelayedprocessingofSEPADirectDebittransactions]]
...
calender days. Dates outside of this range lead to a transaction
...
The provided capture date does not guarantee the money flow on this
...
[[SEPADirectDebit-DifferencesbetweenSEPADirectDebitCore(CoreSDD)andSEPADirectDebitB2B(B2BSDD)Schemes]]
Differences between SEPA _Direct Debit Core_ (Core SDD) and SEPA _Direct
Debit B2B_ (B2B SDD) Schemes
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
3.   Merchant captures mandate data and sends the transaction
information to the WPG
...
2.  Sends signed mandate to his/her bank to authorize the account for
B2B collections
3.   Merchant captures mandate data and sends the transaction
information to the WPG
4.  Merchant stores mandate
...
* Exceptionally small businesses
 ...
 a|
* Businesses only
* Small businesses may be excluded from the scheme on a country by
country basis. Please contact the individual banks.

|*Bank Participation* a|
* Mandatory
...
|*Debit Return* a|
* possible up to 8 weeks after _debit_

* possible up to 13 months after _debit_ (in case of missing mandate)

 a|
* No return possible after _debit_ has been executed
...
|*Mandate Check by Debtor Bank* a|
* Optional
...
1.  Pre-notification: 14 calendar days prior to the due date (D–14):
Merchant notifies the consumer of the upcoming _debit_ transaction. (The
merchant and consumer may agree upon a different timeframe)
2.  Due Date:The transaction must be submitted to the bank earliest one
business day prior to the due date (D–1) for the submission of one-off,
first, and subsequent debits.

 |Same as Core DD
|=======================================================================

For more information regarding the differences between CORE SDD and B2B
SDD, please refer to Annex V in the European Payments Council SEPA
Direct Debit Business to Business Rulebook.

[[SEPADirectDebit-Samples:Request,ResponseandNotification]]
...
*XML Pending-Debit Request (Successful)*  Expand source
...
*XML Pending-Debit Response (Successful)*  Expand source
...
*XML Pending-Debit Request (Failure)*  Expand source
...
*XML Pending-Debit Response (Failed)*  Expand source
...
[[SEPADirectDebit-pending-debitwiththeIBANOnlyfeature.]]
_pending-debit_ with the
https://document-center.wirecard.com/display/PTD/SEPA#SEPA-SEPAIBANOnly[IBAN
Only] feature.
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
[[SEPADirectDebit-<b2b>settotrue]]
<b2b> set to true
^^^^^^^^^^^^^^^^^

*XML debit Request, <b2b> = true (Successful)*  Expand source
...
*XML debit Response, <b2b> = true (Successful)*  Expand source
...
*XML pending-debit Request, <b2b> = true (Successful)*  Expand source
...
*XML pending-debit Response, <b2b> = true (Successful)*  Expand source
...
[[SEPADirectDebit-RecurringTransactions]]
Recurring Transactions
^^^^^^^^^^^^^^^^^^^^^^

*XML Debit Request "First" (Successful)*  Expand source
...
*XML Debit Response "First" (Successful)*  Expand source
...
*XML Debit Request "Recurring" (Successful)*  Expand source
...
*XML Debit Response "Recurring" (Successful)*  Expand source
...
*XML Debit Request "Recurring" (Failure)*  Expand source
...
*XML Debit Response "Recurring" (Failure)*  Expand source
...
[[SEPADirectDebit-authorization.2]]
_authorization_
~~~~~~~~~~~~~~~

*XML Authorization Request (Successful)*  Expand source
...
*XML Authorization Request (Failure)*  Expand source
...
*XML Authorization Response (Failure)*  Expand source
...
[[SEPADirectDebit-void-pending-debit]]
_void-pending-debit_
~~~~~~~~~~~~~~~~~~~~

_void-pending-debit_ transactions can also be used with
https://document-center.wirecard.com/display/PTD/SEPA+Credit+Transfer#SEPACreditTransfer-Void-Pending-Credit[SEPA
Credit Transfer].

*XML Void-Pending-Debit Request (Successful)*  Expand source
...
*XML Void-Pending-Debit Response (Successful)*  Expand source
...
*XML Void-Pending-Debit Request (Failure)*  Expand source
...
*XML Void-Pending-Debit Response (Failure)*  Expand source
...
link:attachments/3703429/3703427.png[20171020_SEPA
DirectDebit_Workflow.png] (image/png) +
image:images/icons/bullet_blue.gif[image,width=8,height=8]
link:attachments/3703429/3703426.png[20171020_SEPA
DirectDebit_Workflow.png] (image/png) +
image:images/icons/bullet_blue.gif[image,width=8,height=8]
link:attachments/3703429/3703428.png[SEPA Sequence Type Flow.png]
(image/png) +

[[footer]]
Document generated by Confluence on Feb 12, 2019 16:43
