  ...
  var
    ...
    utility = {
      ...
      storePageIn: function (object, url, doc, id, state, destUrl) {
        var newDoc = document.implementation.createHTMLDocument("whatever");
    ...
    declaresmoothState = function (options) {
      return this.each(function () {
        ...
        if (this.id && tagname !== 'html' && !$.data(this, 'smoothState')) {
...
function recursiveGetHtmlPageNamesFromTOC(branch) {
  for (var i = 0; i < branch.length; i++) {
    ...
    if (item.attributes.level > 1)
      htmlPageNamesArray.push(pageName);
    else
      htmlPageNamesArray.unshift(pageName);
...
function fillPreloadQueueWithTOC(toc) {
  ...
  /* jshint ignore:start */
  // ignore warning, not used with IE (doesn't have ES6 spread operator)
