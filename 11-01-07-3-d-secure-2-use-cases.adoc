...
. *Check-enrollment* (full amount): This is the initial request and initiates the payment session. If the request is successful, the response contains a _transaction-id._ <<CreditCard_Samples_CheckEnrollment_3DS2, image:images/icons/arrow.png[click here] See sample request/response>>.
