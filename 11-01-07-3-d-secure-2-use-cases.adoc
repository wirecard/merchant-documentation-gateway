...
- In case your merchant account is set up for executing MOTO transactions only, MOTO is assigned as the default commerce type. If the commerce type is not included in the ``payment/entry-mode`` field of the payment request, the respective transaction is marked as MOTO by default.

- In case your merchant account is set up for executing both MOTO transactions and e-commerce transactions, e-commerce is assigned as the default commerce type. To mark a transaction as MOTO, set the ``payment/entry-mode`` field to either ``mail-order`` or ``telephone-order`` in the payment request.
+
====
WARNING: If the commerce type is not included in the ``payment/entry-mode`` field of the payment request, the respective transaction is marked as e-commerce by default and thus subject to SCA.
====
//-

////

are exempted from the PSD2 requirements of Strong Costumer Authentication (SCA). To process MOTO transactions (authorizations and purchases) without SCA, they must be marked accordingly in the card schemes. Your merchant account must be configured for MOTO transactions processing.

If your merchant account is already configured for MOTO transactions, processing is done according to the following rules,corresponding to two available merchant account setup configuration options:

. In the case of merchant accounts setup for executing only MOTO transactions, MOTO is assigned as the default commerce type. If the commerce type is not entered in the ``payment/entry-mode`` field of the payment request, the respective transaction is marked as MOTO by default.

. In the case of merchant accounts setup for executing multiple commerce types transactions, MOTO can be set as a secondary commerce type, while the default type is e-commerce. 
+ 
NOTE: All transactions processed on this type of merchant account, including MOTO transactions, are considered e-commerce by default and, as a result, are subject to the implementation of SCA. To mark a transaction as MOTO, set the ``payment/entry-mode`` field to either ``mail-order`` or ``telephone-order`` in the payment request.
