...
.	*Check-enrollment* (full amount): This is the initial request and initiates the payment session. If the request is successful, the response contains a ``transaction-id``, and the ``three-d/pareq``. The PAReq is returned by the issuer’s ACS via the VISA or Mastercard directory to the Wirecard Payment Gateway, and from here passed on to the agent. The agent uses the PAReq for a HTTPS POST redirect to the ACS URL, which is part of a standard 3D Secure process. The HTTPS POST redirect returns the PARes as part of the response.
.	*Check-payer-response*: This request serves to provide the authentication values needed later on in the *authorization*. The *check-payer-response* is executed after the *check-enrollment* response and the PARes have been received. The *check-payer-response* must contain the following fields
 - ``three-d/pares``: This is the digitally signed, base64-encoded authentication response message received from the issuer.
 - ``parent-transaction-id``: Use the value returned in the ``transaction-id`` field of the *check-enrollment* response. 
The *check-payer-response* includes the ``three-d/cardhoder-authentication-value``.
+
Provide the final authentication values for the merchant. These values include:
+
 - ``three-d/cardhoder-authentication-value`` (Mandatory)
...
.	*Capture* (full amount): Send a request with transaction type *capture.* Include the ``transaction-id`` from the *authorization* response in the ``parent-transaction-id`` field.
