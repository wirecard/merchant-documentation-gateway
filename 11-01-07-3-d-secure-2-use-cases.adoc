...
Mail Order Telephone Order (MOTO) transactions are not considered electronic payments and, as such, are exempted from the PSD2 requirements of Strong Costumer Authentication (SCA). To process MOTO transactions (authorizations and purchases) without SCA, they must be marked accordingly in the card schemes. Your merchant account must be configured for MOTO transactions processing.
